...
export type RecoveryDecision =
	| { readonly kind: "terminal-awaiting-consumption"; readonly task: Task["identity"]; readonly fact: Extract<ProviderFact, { kind: "terminal-winner" }> }
	| { readonly kind: "loss-settled"; readonly task: Task["identity"]; readonly fact: Extract<ProviderFact, { kind: "active-loss" }> }
...
export type GarbageCollectionDecision =
	...
	| { readonly kind: "collected"; readonly closure: ClosureIdentity; readonly publication: Extract<PublicationEvidence, { kind: "published" | "no-work" }> }
	| { readonly kind: "retained"; readonly closure: ClosureIdentity; readonly publication: Extract<PublicationEvidence, { kind: "unpublished" | "unknown" }> }
...
export const RuntimeRecoveryLive: Layer.Layer<RuntimeRecovery, never, ObjectDomainStore | ProviderFactStore | RepositoryGit> = Layer.effect(RuntimeRecovery, Effect.gen(function*() {
	...
	return {
		recoverExpiredLeases: (chains, now) => Effect.map(
			Effect.forEach(chains, (chain) => Effect.flatMap(store.readSnapshot(chain), (snapshot) => Effect.forEach(
				...
				(task) => recoverTask(store, facts, task, now),
...
function recoverTask(store: typeof ObjectDomainStore.Service, facts: typeof ProviderFactStore.Service, task: Task, now: number): Effect.Effect<RecoveryDecision, ObjectStoreError | ProviderFactStoreError> {
	const run = task.state.kind === "leased"
		? task.state.run
		: task.state.kind === "held" && task.state.reason.kind === "unknown-effect"
			? task.state.reason.run
			: null
	if (run === null) return Effect.dieMessage("recoverTask requires an expired lease or unknown-effect hold")
	return Effect.flatMap(facts.read(runProviderFactIdentity(run)), (fact): Effect.Effect<RecoveryDecision, ObjectStoreError> => {
		if (fact?.kind === "terminal-winner") {
			if (task.state.kind === "leased") return Effect.succeed({ kind: "terminal-awaiting-consumption", task: task.identity, fact })
			return Effect.as(
				store.commit({
					identity: `recovery-resume:${runProviderFactIdentity(run)}`,
					transition: { family: "task-resume", task: task.identity, run, resumedAt: now, expiresAt: now + 1 },
				}),
				{ kind: "terminal-awaiting-consumption", task: task.identity, fact },
			)
		}
		if (fact?.kind === "active-loss") {
			return Effect.as(
				store.commit({
					identity: `recovery-loss:${runProviderFactIdentity(run)}`,
					transition: {
						family: "task-settlement",
						task: task.identity,
						run,
						settlement: { kind: "exception", cause: { kind: "exception", cause: { kind: "policy", reason: "program-fault" } }, attempt: run.closure.attempt, closure: run.closure },
						successors: [],
					},
				}),
				{ kind: "loss-settled", task: task.identity, fact },
			)
		}
		...
		return Effect.as(
			store.commit({
				identity: `recovery-hold:${runProviderFactIdentity(run)}`,
...
function freezeTask(store: typeof ObjectDomainStore.Service, repository: typeof RepositoryGit.Service, task: Task): Effect.Effect<GarbageCollectionDecision, ObjectStoreError | GitServiceError> {
	...
	return Effect.flatMap(repository.publication(closure), (publication) => {
		if (publication.kind === "unpublished" || publication.kind === "unknown") {
			return Effect.succeed<GarbageCollectionDecision>({ kind: "retained", closure: closure.identity, publication })
		}
		return Effect.as(
...
function collectTask(store: typeof ObjectDomainStore.Service, repository: typeof RepositoryGit.Service, task: Task): Effect.Effect<GarbageCollectionDecision, ObjectStoreError | GitServiceError> {
	...
	if (publication.kind === "unpublished" || publication.kind === "unknown") return Effect.succeed({ kind: "retained", closure: closure.identity, publication })
	return Effect.as(
		Effect.zipRight(
