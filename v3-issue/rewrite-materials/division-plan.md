# v3 RFC 新划分方案（定稿，v3.3，经六轮对抗）

状态：定稿（第六轮复核通过，无残余发现）。后续各面重写以本文件为划分权威；重写中发现划分错误时另行修订并记录。
...
各面重写：删除与面 0 冲突的旧机制；保留新边界内成立的证据/资源/恢复合同；各面正文普遍采用节末来源标注行，只有关键裁决点（例如面 3 的等待窗口、admit、级联等具体决策）才使用三元组标注，这是主 session 对原约束的收窄裁决；旧 543-548 与新八面的映射表收尾轮统一落一处。【主 session 裁决 | — | 未反驳】
...
| 旧 546 | 面 3（对象域） |
| 旧 547 | 面 1（定义态） |
| 旧 548 | 面 4（入站，对应 A.md 中 `<!-- 548 -->` 标记）+ 面 5（出站，对应 A.md 中 `<!-- F5 -->` 标记，是从旧 548 §9 拆出的部分） |

A.md 中的物理锚点标记沿用旧编号（如 `<!-- 546 -->`），另加一个新增的 `<!-- F5 -->`；其余新八面没有另设新锚点标记。
