...
This fragment defines how the loop state store, loop-control mechanism, per-run stdout JSONL, and handoff files are used.
...
- The loop-control mechanism is owned by the engine. Review requests stop only when no actionable work remains or review infrastructure is broken.
- Iteration stdout JSONL at `chains/<chain>/runs/<runId>/iteration/stdout.jsonl` is the review trace input.
- `chains/<chain>/issues/<issue>.md` is append-only handoff for the selected issue.
- `chains/<chain>/shared.md` stores only stable, source-cited cross-issue facts.
...
- Runtime data, loop-data files, and local evidence must not be staged into feature commits.
