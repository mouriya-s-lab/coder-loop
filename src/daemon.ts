...
const ITEM_ADD_ARG_KEYS = [
	...
	"chainName",
	"name",
	// #419: opaque string item id (replaces `legacyItemNumber` integer wire field).
	"itemId",
	"repoCwd",
	"title",
	"priority",
...
// `legacyItemNumber` wire selector is retired (no compatibility alias — wire is breaking-renamed per
// #456 precedent).
...
export class CoderLoopDaemon {
	...
	private async handleChainCreate(args: JsonObject): Promise<JsonObject> {
		...
		if (repository !== null) input.repository = repository
	...
	private async recordChainStopFromPhaseExitLifecycle(chain: ChainRecord, source: { rowId: number; itemId: string; runId: string; phase: string; subject: ObservabilitySubject; alreadyStopped: boolean; terminatedRunIds: string[] }): Promise<void> {
		await this.recordObservabilityEventIfChainNameIsValid(chain, makeObservabilityEvent({
			...
			payload: {
				chainId: chain.id,
				// `id` here is the opaque preset-declared string id (formerly `legacyItemNumber`).
				id: source.itemId,
	...
	private resolveItem(args: JsonObject): ItemRecord {
		...
		// `legacyItemNumber` integer selector is retired (breaking wire change, #456 precedent).
...
function chainCreateConflicts(existing: ChainRecord, requested: CreateChainInput): JsonObject[] {
	...
	if (requested.repository !== undefined) {
		addConflict("metadata.bindings.repository", chainBindings(existing.metadata).repository ?? null, requested.repository)
	}
...
// #419: now keys on the opaque preset-declared `itemId` string (formerly `legacyItemNumber`
...
function itemToJson(item: ItemRecord, dependencyWait?: DependencyWaitReason | null): JsonObject {
	// #419: wire schema retires top-level `legacyItemNumber` (int), `branch`, `pr`. The opaque
