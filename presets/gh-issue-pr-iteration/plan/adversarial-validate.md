...
6. **Spike-specific adversarial**:
	   - For comment-spike-deliverable issues, simulate the laziest spike comment. Does it select a `## 结果分支` line AND propose the minimum sub-issue titles?
	   - For source-writing-spike-deliverable issues, simulate the laziest source-writing spike. Does it write real PoC/runtime evidence, post a no-merge issue comment, and avoid PR creation/merge?
...
7. **Unblock-specific adversarial**. For unblock-deliverable issues, simulate the laziest unblock PR. If it could pass by changing code plus unit tests without replaying the blocked path, add or sharpen an e2e/integration checkpoint row. If `Unblocks:` is absent or ambiguous, fix the issue body before creation.
