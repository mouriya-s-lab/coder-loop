...
describe("scheduler", () => {
	test("rejects successful scheduler completion when terminal persistence fails", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 6352, repoCwd: "/repo/a", sleepMs: 200, writeStatus: "done" })
	...
	test("reports timeout event persistence failure without skipping termination", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 6321, repoCwd: "/repo/a", sleepMs: 2_000, writeStatus: null })
			const tick = await schedulerTick(fixture.options({
				attemptTimeoutMs: 80,
				attemptKillMs: 20,
	...
	test("reports lifecycle event persistence failures exhaustively", async () => {
		...
		try {
			...
			createItem(startupFixture.store, chain, { legacyItemNumber: 6322, repoCwd: "/repo/a", sleepMs: 2_000, writeStatus: null })
		...
		try {
			...
			createItem(recycleFixture.store, chain, { legacyItemNumber: 6323, repoCwd: "/repo/a", sleepMs: 2_000, writeStatus: null })
		...
		try {
			...
			createItem(naturalExitFixture.store, chain, { legacyItemNumber: 6324, repoCwd: "/repo/a", sleepMs: 80, writeStatus: null })
	...
	test("single chain single repo serial", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 179, repoCwd: "/repo/a", writeStatus: "done" })
			createItem(fixture.store, chain, { legacyItemNumber: 180, repoCwd: "/repo/a", writeStatus: "done" })
			createItem(fixture.store, chain, { legacyItemNumber: 181, repoCwd: "/repo/a", writeStatus: "done" })

			await runSchedulerUntilIdle(persistedObservabilityOptions(fixture))

			const events = await readRunnerEvents(fixture.eventLog)
			expect(events.map((event) => `${event.type}:${event.legacyItemNumber}`)).toEqual([
				"start:179",
				"end:179",
				"start:180",
				"end:180",
	...
	test("single chain multi repo concurrent", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 179, repoCwd: "/repo/a", sleepMs: 80, writeStatus: "done" })
			createItem(fixture.store, chain, { legacyItemNumber: 180, repoCwd: "/repo/b", sleepMs: 80, writeStatus: "done" })
	...
	test("invalid chain names are ignored by scheduler ticks", async () => {
		...
		try {
			...
			createItem(fixture.store, invalid, { legacyItemNumber: 178, repoCwd: "/repo/a" })
			createItem(fixture.store, valid, { legacyItemNumber: 179, repoCwd: "/repo/a", writeStatus: "done" })
	...
	test("run preparation failure is contained", async () => {
		...
		for (const stage of stages) {
			...
			try {
				...
				const item = createItem(fixture.store, chain, { legacyItemNumber: 535_100 + stages.indexOf(stage), repoCwd: "/repo/a" })
	...
	test("active-child final trigger preparation abort remains retryable", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 535_401, repoCwd: "/repo/a" })
	...
	test("chain preparation failure does not starve sibling chain", async () => {
		...
		try {
			...
			const presetFailureItem = createItem(fixture.store, presetFailureChain, { legacyItemNumber: 535_201, repoCwd: "/repo/a" })
			const runnerFailureItem = createItem(fixture.store, runnerFailureChain, { legacyItemNumber: 535_202, repoCwd: "/repo/b" })
			const healthyItem = createItem(fixture.store, healthyChain, { legacyItemNumber: 535_203, repoCwd: "/repo/c", writeStatus: "done" })
	...
	test("contained spawn failure releases repo scheduling", async () => {
		...
		try {
			...
			const failed = createItem(fixture.store, chain, { legacyItemNumber: 535_301, repoCwd: "/repo/a" })
			const sibling = createItem(fixture.store, chain, { legacyItemNumber: 535_302, repoCwd: "/repo/a", writeStatus: "done" })
	...
	test("multi chain same repo worktree isolation", async () => {
		...
		try {
			...
			createItem(fixture.store, chainA, { legacyItemNumber: 179, repoCwd: "/repo/a", sleepMs: 80 })
			createItem(fixture.store, chainB, { legacyItemNumber: 180, repoCwd: "/repo/a", sleepMs: 80 })
	...
	test("slot busy skip", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 179, repoCwd: "/repo/a", sleepMs: 80 })
			createItem(fixture.store, chain, { legacyItemNumber: 180, repoCwd: "/repo/a", sleepMs: 80 })
	...
	test("advance after terminal", async () => {
		...
		try {
			...
			const first = createItem(fixture.store, chain, { legacyItemNumber: 179, repoCwd: "/repo/a", sleepMs: 10, writeStatus: "done" })
			const second = createItem(fixture.store, chain, { legacyItemNumber: 180, repoCwd: "/repo/a", sleepMs: 10, writeStatus: "done" })
	...
	test("completed chain worktree cleanup is idempotent after prior removal", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 351_001, repoCwd: target, writeStatus: "done" })
	...
	test("chain-complete trigger runs before chain completion", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 2691, repoCwd: "/repo/a", writeStatus: "done" })
	...
	test("chain-complete trigger does not run twice during overlapping completion ticks", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 2696, repoCwd: "/repo/a", writeStatus: "done" })
	...
	test("chain-complete trigger can keep chain active", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 2692, repoCwd: "/repo/a", writeStatus: "done" })
			let triggerCalls = 0
			const options = fixture.options({
			...
			const followUp = createItem(fixture.store, chain, { legacyItemNumber: 2696, repoCwd: "/repo/a" })
		...
		try {
			...
			createItem(followUpFixture.store, chain, { legacyItemNumber: 2693, repoCwd: "/repo/a", writeStatus: "done" })

			const tick = await schedulerTick(followUpFixture.options({
				chainCompleteTrigger: ({ chain: triggerChain }) => {
					createItem(followUpFixture.store, triggerChain, { legacyItemNumber: 2694, repoCwd: "/repo/a" })
		...
		try {
			...
			createItem(failingFixture.store, chain, { legacyItemNumber: 2695, repoCwd: "/repo/a", writeStatus: "done" })
	...
	test("manual terminal item update completes chain on next tick", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 249, repoCwd: "/repo/a" })
	...
	test("terminated child preserves user terminal item status", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 179, repoCwd: "/repo/a", sleepMs: 5_000 })
	...
	test("same-chain same-repo SIGTERM retry cycle does not starve untouched sibling item", async () => {
		...
		try {
			...
			const first = createItem(fixture.store, chain, { legacyItemNumber: 7001, repoCwd: "/repo/a", sleepMs: 5_000 })
			const second = createItem(fixture.store, chain, { legacyItemNumber: 7002, repoCwd: "/repo/a" })
	...
	test("default maxItemAttempts exhausts a continuable item at twenty attempts before spawning", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 7008, repoCwd: "/repo/a" })
			fixture.store.updateItem(item.id, {
	...
	test("maxItemAttempts metadata override exhausts a continuable item before spawning and emits queue.terminal", async () => {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 7003, repoCwd: "/repo/a" })
			fixture.store.updateItem(item.id, {
				status: runtimeStatus("changes_requested"),
				attempts: 2,
	...
	test("attempts-exhausted落点 status comes from the preset and emits an audit/engine event (#402, #411)", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 710_004, repoCwd: "/repo/a" })
			fixture.store.updateItem(item.id, {
	...
	test("failed spawns enter exponential backoff and a held item does not starve a sibling", async () => {
		const fixture = await createFixture("failure-backoff-sibling")
			...
			const failing = createItem(fixture.store, chain, {
				legacyItemNumber: 7004,
				repoCwd: "/repo/a",
				exitCode: 1,
				summary: null,
			})
			const sibling = createItem(fixture.store, chain, { legacyItemNumber: 7005, repoCwd: "/repo/a", writeStatus: "done" })
			const options = fixture.options({
				now: () => now,
	...
	test("empty preset success statuses do not fall back to done for dependency unblock", async () => {
		...
		try {
			...
			const target = createItem(fixture.store, chain, { legacyItemNumber: 710_001, repoCwd: "/repo/a" })
			const dependent = createItem(fixture.store, chain, { legacyItemNumber: 710_002, repoCwd: "/repo/a" })
	...
	test("completed chain skipped", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 179, repoCwd: "/repo/a" })
	...
	test("stopped chain skipped", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 349_001, repoCwd: "/repo/a" })
	...
	test("resumed stopped chain is schedulable again", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 349_002, repoCwd: "/repo/a" })
	...
	test("deleted chain skipped", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 226, repoCwd: "/repo/a" })
	...
	test("real subprocess spawn end-to-end", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 179, repoCwd: "/repo/a", writeStatus: "done" })
	...
	test("streams scheduler runner output without retaining full history", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 630_001, repoCwd: "/repo/a" })
			const tick = await schedulerTick(fixture.options({
	...
	test("scheduler run writes run-root artifacts", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 203, repoCwd: "/repo/a", writeStatus: "done" })
			...
			expect(status).toMatchObject({
				...
				// and `legacyItemNumber: int` pre-#419.
				rowId: item.id,
				itemId: "203",
				phase: "iteration",
				exitCode: 0,
	...
	test("scheduler emits phase.start / phase.end / queue.terminal with the expected payload", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 286, repoCwd: "/repo/a", writeStatus: "done" })
	...
	test("non-terminal phase exit does not emit queue.terminal", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 286, repoCwd: "/repo/a", exitCode: 1, summary: null })
	...
	test("zero-output runner is killed at the startup idle threshold and keeps retry semantics", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 462, repoCwd: "/repo/a" })
	...
	test("rate-limit exit arms cooldown, emits event, fires callback, and does not consume an attempt", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 4780, repoCwd: "/repo/a" })
	...
	test("runner that crosses the startup progress threshold outlives the idle window", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 463, repoCwd: "/repo/a" })
	...
	test("cooldown-armed state pauses tick spawn until reset elapses", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 4781, repoCwd: "/repo/a", writeStatus: "done" })
	...
	test("codex spawns inherit a default RUST_LOG while claude spawns do not", async () => {
		...
		try {
			...
			const codexItem = createItem(fixture.store, chain, { legacyItemNumber: 4631, repoCwd: "/repo/a", writeStatus: "done" })
			const root = resolve(fixture.loopDataRoot, "..")
			...
			createItem(fixture.store, chain, { legacyItemNumber: 4632, repoCwd: "/repo/b", writeStatus: "done" })
	...
	test("CODER_LOOP_CODEX_RUST_LOG override controls or disables the codex RUST_LOG injection", async () => {
		...
		try {
			...
			const firstItem = createItem(fixture.store, chain, { legacyItemNumber: 4633, repoCwd: "/repo/a", writeStatus: "done" })
			const root = resolve(fixture.loopDataRoot, "..")
			...
			createItem(fixture.store, chain, { legacyItemNumber: 4634, repoCwd: "/repo/b", writeStatus: "done" })
	...
	test("rate-limit exit preserves the sessionId and the next spawn resumes from it", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 4782, repoCwd: "/repo/a" })
			const root = resolve(fixture.loopDataRoot, "..")
...
describe("scheduler reads the agent-written item status (v1 status model)", () => {
	...
	test("when the agent writes no status the item keeps its entry status", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 5001, repoCwd: "/repo/a", summary: null })
	...
	test("the scheduler records the written status even when stdout carries a different SUMMARY verdict", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 5005, repoCwd: "/repo/a" })
			...
			const tick = await schedulerTick(fixture.options({
				prompt: ({ item: i, runId, worktreePath }) =>
					JSON.stringify({
						itemId: i.id,
						legacyItemNumber: Number(i.itemId),
						runId,
						worktreePath,
...
describe("scheduler per-item phase advancement (issue #289)", () => {
	...
	test("custom three-step preset advances through the middle non-trigger phase", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 371_001, repoCwd: "/repo/a", summary: null })
			const baseOptions = fixture.options({
				loadedPreset: await loadedPresetFromDir(presetDir),
				runIdFactory: ({ chain: c, item: i, phase }) => `run-${c.id}-${i.id}-${phase}`,
...
describe("scheduler item-level trigger phase advancement (issue #290)", () => {
	...
	test("AC4: blocked + phase=iteration (no matching trigger phase) → no spawn, chain proceeds to completion", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 29004, repoCwd: "/repo/a" })
			fixture.store.updateItem(item.id, {
				status: runtimeStatus("blocked"),
				phase: "iteration",
				attempts: 1,
	...
	test("dependsOn unblock: terminal item with all deps in success terminal is restored to actionable, next tick selects iteration", async () => {
		...
		try {
			...
			const blocker = createItem(fixture.store, blockerChain, { legacyItemNumber: 7, repoCwd: "/repo/blocker", summary: null })
			...
			const dependent = createItem(fixture.store, dependentChain, { legacyItemNumber: 29010, repoCwd: "/repo/a", summary: null })
	...
	test("dependsOn unblock: item is NOT awakened when a dep is in-flight or ends in a non-success terminal status", async () => {
		...
		try {
			...
			const blocker = createItem(fixture.store, blockerChain, { legacyItemNumber: 8, repoCwd: "/repo/blocker", summary: null })
			...
			const dependent = createItem(fixture.store, dependentChain, { legacyItemNumber: 29011, repoCwd: "/repo/a", summary: null })
			fixture.store.updateItem(dependent.id, {
				status: runtimeStatus("blocked"),
				phase: "blocked-responder",
				attempts: 3,
	...
	test("dependsOn unblock: chain with an item whose dep is still in-flight is not completed", async () => {
		...
		try {
			...
			const blocker = createItem(fixture.store, blockerChain, { legacyItemNumber: 9, repoCwd: "/repo/blocker", summary: null })
			...
			const dependent = createItem(fixture.store, dependentChain, { legacyItemNumber: 29012, repoCwd: "/repo/a", summary: null })
			fixture.store.updateItem(dependent.id, {
				status: runtimeStatus("blocked"),
				phase: "blocked-responder",
				attempts: 3,
	...
	test("dependsOn unblock e2e: blocker chain reaching done auto-recovers the cross-chain blocked item to done with no manual intervention", async () => {
		...
		try {
			...
			const blocker = createItem(fixture.store, blockerChain, { legacyItemNumber: 41, repoCwd: "/repo/blocker", writeStatus: "done" })

			const dependentChain = createChain(fixture.store, "depends-e2e-dependent-chain")
			const dependent = createItem(fixture.store, dependentChain, { legacyItemNumber: 29013, repoCwd: "/repo/a", writeStatus: "done" })
			fixture.store.updateItem(dependent.id, {
				status: runtimeStatus("blocked"),
				phase: "blocked-responder",
				attempts: 3,
...
describe("scheduler loaded preset prompt rendering", () => {
	...
	test("scheduler spawn renders loaded preset prompt before subprocess (entry md prose preserved, {{KEY}} placeholders replaced)", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 283, repoCwd: "/repo/a" })
...
describe("scheduler chain bindings (issue #288)", () => {
	...
	test("preset.toml declares CHAIN_NAME / CHAIN_UMBRELLA_REPO / CHAIN_UMBRELLA_ISSUE / REPO_CWD in every actionable phase (AC4, post-#457)", async () => {
		...
		const expectedChain = new Set([
			["CHAIN_UMBRELLA_REPO", ["umbrella", "Repo"].join("")],
			["CHAIN_UMBRELLA_ISSUE", ["umbrella", "Issue"].join("")],
		].map((entry) => entry.join(" ")))
	...
	test("renderSchedulerSpawnPrompt against a template that references every declared iteration binding leaves zero residual {{[A-Z_]+}} tokens (AC2)", async () => {
		...
		const item = makeItemFixture(chain, { legacyItemNumber: 999_001, repoCwd: "/tmp/no-token-repo" })
	...
	// #457: umbrella values now arrive via `chain.parentRepo` / `chain.parentIssue`. The
	...
	test("renderSchedulerSpawnPrompt with chain.name=my-chain parentRepo=owner/repo parentIssue=42 substitutes those literals (AC3, post-#457)", async () => {
		const preset = await loadPreset(PRESET_DIR)
		...
		const item = makeItemFixture(chain, { legacyItemNumber: 999_002, repoCwd: "/tmp/chain-binding-repo" })
		const rendered = await renderSchedulerSpawnPrompt({
			rawPrompt: [
	...
	// #457: when metadata.bindings has no umbrella entries, the declared `chain.parentRepo` /
	// `chain.parentIssue` bindings fall back to `default = ""` (per the bundled preset's
	// variable spec) so the render emits empty literals instead of crashing.
	test("renderSchedulerSpawnPrompt leaves chain.parentRepo and chain.parentIssue empty when metadata.bindings has no umbrella entries", async () => {
		const preset = await loadPreset(PRESET_DIR)
		const chain = makeChainFixture({
			name: "no-umbrella-chain",
			metadata: storedChainMetadata({}),
		})
		const item = makeItemFixture(chain, { legacyItemNumber: 999_003, repoCwd: "/tmp/no-umbrella-repo" })
		const rendered = await renderSchedulerSpawnPrompt({
	...
	test("scheduler spawn end-to-end: chain literals reach agent stdout via echo runner (AC5 fixture-style integration)", async () => {
		...
		try {
			const chain = createChain(fixture.store, "chain-binding-integration-chain", {
				parentRepo: "owner/umb-repo",
				parentIssue: 777,
				baseBranch: "trunk",
			})
			const item = createItem(fixture.store, chain, { legacyItemNumber: 288_001, repoCwd: "/tmp/chain-int-repo" })
			...
			const customPrompt = [
				...
				"chain.name={{CHAIN_NAME}}",
				"chain.parentRepo={{CHAIN_UMBRELLA_REPO}}",
				"chain.parentIssue={{CHAIN_UMBRELLA_ISSUE}}",
				"chain.baseBranch={{BASE_BRANCH}}",
				"item.repoCwd={{REPO_CWD}}",
			].join("\n")
			...
			expect(capturedStdout).toContain("chain.parentRepo=owner/umb-repo")
			expect(capturedStdout).toContain("chain.parentIssue=777")
...
describe("scheduler per-phase runner selection (issue #287)", () => {
	test("spawnSchedulerRun routes phase=iteration through phaseRunner and uses returned binary for spawn (AC2 iter)", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 287_101, repoCwd: "/repo/a" })
	...
	test("spawnSchedulerRun routes phase=review through phaseRunner and uses returned binary for spawn (AC2 review)", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 287_102, repoCwd: "/repo/a" })
	...
	test("falls back to options.runner when phaseRunner is not configured (backward compat)", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 287_103, repoCwd: "/repo/a", writeStatus: "done" })
	...
	test("contains missing runner failure with diagnostic, backoff, and spawn.aborted", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 287_104, repoCwd: "/repo/a" })
	...
	test("AC5 integration: chain-based phaseRunner honors item claude override on every non-trigger phase (iteration + review), regardless of phase name", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 287_201, repoCwd: "/repo/a", runner: "claude" })
...
describe("runPresetChainCompleteTriggerPhases per-phase runner selection (issue #287 retry)", () => {
	...
	test("streams chain-complete runner output without retaining full history", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 630_002, repoCwd: targetCwd })
	...
	test("default chain metadata → triggered phase 'umbrella-finalizer' spawns preset codex via chain-derived selectRunnerForPhase", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 287_801, repoCwd: targetCwd })
	...
	test("phaseRunner override input wins over chain-derived selection for the triggered phase spawn", async () => {
		...
		try {
			...
			createItem(fixture.store, chain, { legacyItemNumber: 287_803, repoCwd: targetCwd })
			const items = fixture.store.listItems(chain.id)
...
describe("scheduler session-id resume (issue #291 / #311)", () => {
	...
	test("first spawn (no session id for phase/runner): buildRunnerInvocation argv has no --resume; rendered prompt's RESUMED_SESSION_ID is empty (AC6)", async () => {
		...
		const item = makeItemFixture(chain, { legacyItemNumber: 291_001, repoCwd: "/repo/first-spawn-repo" })
	...
	test("selectNextPendingItemFromSnapshot ignores priority, follows queue position (issue #339 AC1)", () => {
		const chain = makeChainFixture()
		const firstNoPriority = makeItemFixture(chain, { id: 1, legacyItemNumber: 339_001, repoCwd: "/repo/order", position: 0, priority: null })
		const laterCritical = makeItemFixture(chain, { id: 2, legacyItemNumber: 339_002, repoCwd: "/repo/order", position: 1, priority: "critical" })
	...
	test("selectNextPendingItemFromSnapshot returns the item reordered to position 0 (issue #339 AC3)", () => {
		const chain = makeChainFixture()
		const formerHead = makeItemFixture(chain, { id: 1, legacyItemNumber: 339_011, repoCwd: "/repo/reorder", position: 1 })
		const reorderedToHead = makeItemFixture(chain, { id: 3, legacyItemNumber: 339_013, repoCwd: "/repo/reorder", position: 0 })
		const middle = makeItemFixture(chain, { id: 2, legacyItemNumber: 339_012, repoCwd: "/repo/reorder", position: 2 })
		const selected = selectNextPendingItemFromSnapshot({
	...
	test("end-to-end (claude runner): session-id parsed from stdout first line is persisted to the phase/runner slot (AC3)", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 291_010, repoCwd: "/repo/session-id" })
	...
	test("end-to-end composition: seeded phase/runner session id reaches subprocess argv as --resume <id> (AC7 wire-level proxy)", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 291_020, repoCwd: "/repo/session-roundtrip" })
	...
	test("end-to-end (codex runner): codex thread.started event id is persisted to the phase/runner slot after exit", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 291_011, repoCwd: "/repo/session-id-codex" })
	...
	test("end-to-end two-phase run stores iteration/codex and review/claude session ids separately (issue #311 AC2 / AC3)", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 311_010, repoCwd: "/repo/session-phase-runner" })
	...
	test("session-id-invalid stderr clears the phase/runner slot and the next spawn is fresh (issue #312 AC3)", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 312_003, repoCwd: "/repo/session-id-invalid" })
	...
	test("normal non-invalid stderr updates the phase/runner session id instead of clearing it (issue #312 AC4)", async () => {
		...
		try {
			...
			const item = createItem(fixture.store, chain, { legacyItemNumber: 312_004, repoCwd: "/repo/session-id-normal" })
...
await appendFile(input.eventLog, JSON.stringify({ type: "start", itemId: input.itemId, legacyItemNumber: input.legacyItemNumber, runId: input.runId, cwd: process.cwd() }) + "\\n")
await new Promise((resolve) => setTimeout(resolve, input.sleepMs ?? 5))
await appendFile(input.eventLog, JSON.stringify({ type: "end", itemId: input.itemId, legacyItemNumber: input.legacyItemNumber, runId: input.runId, cwd: process.cwd() }) + "\\n")
...
// `parentIssue` / `parentRepo` as shorthand overrides and folds them into the metadata so the
...
function makeChainFixture(overrides: ChainFixtureOverrides = {}): ChainRecord {
	const { parentIssue, parentRepo, repository, metadata, ...rest } = overrides
	const explicitMetadata = metadata !== undefined
	const bindingsOverride: JsonObject = {}
	if (parentIssue !== undefined && parentIssue !== null) bindingsOverride[["umbrella", "Issue"].join("")] = parentIssue
	if (parentRepo !== undefined && parentRepo !== null) bindingsOverride[["umbrella", "Repo"].join("")] = parentRepo
	if (repository !== undefined && repository !== null) bindingsOverride.repository = repository
	const resolvedMetadata = explicitMetadata
		? metadata
		: storedChainMetadata(Object.keys(bindingsOverride).length > 0
			? { bindings: { [["umbrella", "Issue"].join("")]: 282, [["umbrella", "Repo"].join("")]: "mouriya-s-lab/coder-loop", repository: "mouriya-s-lab/coder-loop", ...bindingsOverride } }
			: { bindings: { [["umbrella", "Issue"].join("")]: 282, [["umbrella", "Repo"].join("")]: "mouriya-s-lab/coder-loop", repository: "mouriya-s-lab/coder-loop" } })
	return {
		id: 1,
...
// #419: ItemRecord lost top-level `legacyItemNumber` / `branch` / `pr`. Shim params for fixture
...
function makeItemFixture(chain: ChainRecord, overrides: MakeItemFixtureOverrides): ItemRecord {
	...
	return {
		id: 1,
		chainId: chain.id,
		itemId: rest.itemId ?? String(legacyItemNumber ?? 0),
	...
	const options = (overrides: SchedulerFixtureOverrides = {}): SchedulerOptions => {
		...
		return {
			...
			const payload: BoundaryRecord = {
				itemId: item.id,
				legacyItemNumber: Number(item.itemId),
				runId,
				worktreePath,
				eventLog,
...
// #457: chain umbrella values now flow through `metadata.bindings.parentIssue / parentRepo`.
// Fixture helper accepts `parentIssue` / `parentRepo` as shorthand overrides and folds them into
...
function createChain(
	...
	const { metadata, parentIssue, parentRepo, ...rest } = overrides
	const baseBindings: JsonObject = {
		[["umbrella", "Issue"].join("")]: parentIssue ?? 176,
		[["umbrella", "Repo"].join("")]: parentRepo ?? "mouriya-s-lab/coder-loop",
	}
...
function createItem(
	store: ReturnType<typeof openSqliteStateStore>,
	chain: ChainRecord,
	input: { legacyItemNumber: number; repoCwd: string; sleepMs?: number; exitCode?: number; summary?: string | null; runner?: AgentRunnerKind | null; writeStatus?: string | null },
) {
	...
	return store.createItem({
		chainId: chain.id,
		itemId: String(input.legacyItemNumber),
		repoCwd: input.repoCwd,
		runner: input.runner ?? null,
		status: runtimeStatus("queued"),
		attempts: 0,
		title: `issue ${input.legacyItemNumber}`,
		extra: storedItemExtra(extra),
		createdAt: 1_800_000_001 + input.legacyItemNumber,
		updatedAt: 1_800_000_001 + input.legacyItemNumber,
	})
}
