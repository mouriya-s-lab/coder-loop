...
| `[[phases.hostResourceBindings]]` | `{ name, resource }[]` | 否 | 声明 phase 内具名临界区到主机资源键的绑定。agent 用 `coder-loop resource run <name> -- <command...>` 只包围实际重量命令；同一 `resource` 跨 phase/chain 互斥，wait/acquire/release 进入 status/log/trace。 |
