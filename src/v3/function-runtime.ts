...
export type FunctionExecutionResult =
	| { readonly kind: "settled"; readonly commit: CommitResult; readonly settlement: TaskSettlement; readonly hooks: readonly HookDispatchObservation[] }
	...
	return Layer.effect(FunctionRuntime, Effect.gen(function*() {
		...
		const execute = (identity: TaskIdentity): Effect.Effect<FunctionExecutionResult, FunctionRuntimeError> => Effect.gen(function*() {
			...
			while (true) {
				...
				return yield* commitSettlement(store, hooks, hooksObserved, task, run, settlement, successors)
			}
		})
...
function settleException(
	...
	return commitSettlement(store, hooks, observed, task, run, settlement, [])
}
...
function commitSettlement(
	...
	return Effect.gen(function*() {
		...
		return { kind: "settled", commit, settlement, hooks: observed }
	})
}
