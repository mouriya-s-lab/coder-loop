...
The e2e runbook for one coder-loop iteration. Iteration executes this step inline in its own session — this preset forbids subagents, so treat the sections below as your own instruction set, not a task spec for a nested agent. The deliverable is the formal deliverable evidence: the real thing run directly and one explicitly typed runtime handoff. Unit/integration results and acceptance-row outputs from the preceding verify step are supporting layers only.

## Task

From the iteration's runtime bindings and your Step focus: `ISSUE`, `REPO`, `RUN_ID`, `AGENT_CWD` (the issue-branch checkout — verify ran there first, no concurrent execution here since subagents are forbidden; Step 2 gives you your own worktree for browser / service work), `EVIDENCE_DIR`, `REQUIRE_BROWSER_EVIDENCE`, and `Step focus` — the changed path to exercise and the marker Checks whose `Kind` is `browser` (iteration enumerates them by stable ID; verify deferred the same checks to you). Read now, before Step 1: `{{PRESET_ROOT}}/quality/evidence.md` and `{{PRESET_ROOT}}/quality/cleanup.md` — they bind every run and side effect below, including the two-case auth rule.
...
- **Deferred Checks closed** — every `Kind=browser` stable ID from the `Step focus` appears here with an observed-vs-expected verdict from the real UI walk (and against the verify step's deferred set: same IDs by construction). A failing Check is a product failure: iteration inserts a new implement step (fix), then re-runs verify then e2e sequentially for the full contract.
