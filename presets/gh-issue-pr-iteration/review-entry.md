...
MUST NOT: repair iteration output (code, evidence, PR body); produce a verdict on a PR-backed route without both mandatory dispatched reports accepted; merge a PR before Steps 3–5 pass; close an issue whose required external effects have not all succeeded; edit merged PR bodies; create child issues except through the expand-parent action; bypass the daemon-serialized CLI for state writes; remove central daemon scheduling state outside the Step 7 rules. No internal timeouts anywhere — the engine watchdog owns time. Code style, naming, conventions, architecture taste, and bug-hunting beyond the issue contract are out of loop scope (later human review); scope mapping, test integrity, hygiene, contract rows, and CI reality are in scope and non-negotiable.
