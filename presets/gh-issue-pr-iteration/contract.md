...
- 写 queue 进 `central state DB` 后必须跑 `coder-loop status <target> --json --chain <chain-name>` exit 0，并确认 `.state.kind == "ok"` 与 `.queue.selected.id` 指向预期 next item，否则 plan/init-queue 不能 verdict pass。
