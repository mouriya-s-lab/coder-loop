...
export type SchedulerEvent =
	...
	// #419: `spawn.aborted` payload retires the integer `legacyItemNumber` field. The `id` here is the
	// item's opaque preset-declared string id (formerly `legacyItemNumber`-as-int). `itemId` remains
...
function chainCompletionFingerprint(chain: ChainRecord, items: readonly ItemRecord[], terminalStatuses: readonly InternalStatus[]): string {
	const payload: JsonObject = {
		...
		items: items
			.map((item) => ({
				id: item.id,
				// #419: fingerprint replaces `legacyItemNumber` (integer) with the opaque preset
