...
6. **Land the artifacts and report.** Everything lands under `EVIDENCE_DIR`. You had no reason to commit, push, open PRs, or write GitHub/queue state — confirm you did not. Report per the Report section, mismatches reported as mismatches without softening. (The e2e direct run, the runtime manifest, and the standing environment belong to the e2e step.)
