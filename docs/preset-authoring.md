...
`presets/single-phase-example/preset.toml`（`[agent]` 只支持 `attemptTimeoutSeconds`；runner binary 由 phase runner 决定）：
...
| `[phases.rights]` | table | 否 | 该 phase 持凭据的 agent 调用 mutation CLI 时的准入授权。`createItems = true` 允许 `item.add`/`item.batch-add`（跨 chain 添加时按新 item 所属 preset 的同名 phase 校验）；`writableFields = [...]` 列出 `item.update --field-json` 可写的透明字段（control-plane 字段 `repoCwd`/`runner`/`dependsOn`/`priority` 不可经此授予）；`privilegedOps = [...]` 的词表是 `"item.reorder"` 与 `"item.dependsOn"` —— 前者放行 `item.reorder`，后者按字段轴放行 `item.update` 写自己 item 的顶层 `dependsOn`（仍过跨 chain dependsOn 图校验：目标必须存在、无环）。未声明 rights 的 phase 一律拒绝这三类 mutation。 |
...
- preset 声明任何 `[[fragments]]` 时，每个 `[[phases]]` 必须声明 `roles` 数组（phase↔role 映射必须显式，引擎不按 phase 名推断）；数组内每个 role 必须出现在某个 `[[fragments]].role` 里，且不可重复；preset 无 fragments 时可省 `roles`；
...
| `chain.<field>` | centralized chain.metadata.bindings 字段（chain 创建时通过 `coder-loop chain create --config-json '{...}'` 写入） | 字段不存在 → throw；`null/undefined` → throw；类型同上 |
...
Preset business key 通过 `preset.toml` 两步声明 + 供值：
...
| `statusVocabularyDoc` | 按 phase 切片的 status 词表（`actionable` / `non-actionable` 分组）；trigger phase 只渲染 `whenStatus`，普通工作 phase 渲染 continuable，含 `[[phases.exits]]` 的 phase（如 review）渲染 continuable + terminal。 |
| `triggerStatusDoc` | 当前 phase 的 trigger 关注 status 字面量（仅 trigger phase 非 chain-complete 时非空）。 |
| `terminalStatusesDoc` | `preset.statuses.terminal` 的逗号分隔字面表，供 phase prose 引用「不可写入的终态集合」。 |
| `retryStatusDoc` | preset 声明的 retry continuable status（`[statuses].retry`）加 backtick；未声明则为空串。 |
...
> `baseBranch` 是 chain 一等列（worktree 创建机制消费）；prompt 业务侧通过 `chain.baseBranch` 命名空间读取（`buildSchedulerChainBindings` 的常规展开）。umbrella 值是 chain 声明字段（`chain.umbrellaRepo` / `chain.umbrellaIssue`），存在 `metadata.bindings` 里，bundled `gh-issue-pr-iteration` preset 通过 `chain.<field>` 读取并带 `default = ""` fallback。引擎不注入这些作为 runtime fact。

Bundled `gh-issue-pr-iteration` 当前不声明任何 business key——preset 不消费 kind 信号，kind 分类由 preset prompt 自己在 issue label 与 body 上判断。
...
Runner 与 model 默认值写在 `preset.toml`。每个 phase 可声明：
...
引擎不要求 entry prompt 用任何特定结构。`gh-issue-pr-iteration` 当前用**单 session workflow** 约定（本 preset 禁用 subagent）：每个 phase 的 entry md 是按序编号的 workflow（不是散文手册），agent 在自己那一次 spawn 里 inline 走完全部步骤。iteration 额外维护显式任务清单（两态出口 `[x]` / `[-] skipped: <reason>`，全勾完才能退出），每步读单文件 runbook（`iter/steps/<name>.md`，内含 `Task` / `Report` / `Acceptance` 三段——Task 是自己的动作序列、Report 是 handoff 备注形状、Acceptance 是勾行前的自检门槛；判据对执行者可见是有意设计，前提是诚实 runner；见 `presets/gh-issue-pr-iteration/DESIGN.md`）。质量判据是单文件（`quality/evidence.md` / `quality/honesty.md` / `quality/cleanup.md`），执行侧约束与判断侧规则同源同文。跨 phase 交接走 GitHub durable packet（`common/packets.md`），不走 session 内转述。注意：fragment 文件不经引擎 `{{KEY}}` 渲染，跨文件引用用引擎自有词表 `{{PRESET_ROOT}}/<rel>`——`loadPreset` 时物化层把整个 preset 目录复制到 `<loopDataRoot>/preset-materialized/<name>-<hash>/`，同一步把每个 `.md` 里的 `{{PRESET_ROOT}}` 按字面串替换为该副本的绝对路径；agent 拿到的是稳定可读的运行时绝对路径。语义登记在 `docs/reserved-strings.md`。
