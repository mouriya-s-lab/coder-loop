...
| 3 doctor | `coder-loop doctor <target> --repo <owner>/<repo>` | operator 机器先决条件 + live runtime 全 OK | live runtime 有 WARN（current 缺等可解释信号） | PATH / gh auth / runner CLI 缺失，或 live runtime FAIL |
