...
`)
	...
	test("socket chain.create is idempotent but rejects conflicting existing fields", async () => {
		...
		try {
			...
			expect(record(listedChain)).toMatchObject({ baseBranch: "main", metadata: { codex: { binary: "codex" } } })
		} finally {
			await fixture.daemon.stop()
		}
	})
	...
	test("socket chain.create accepts repository as opaque binding sugar", async () => {
		const fixture = await startFixture("chain-create-invalid-repository", { schedulerEnabled: false })
		try {
			const response = expectOk(await request(fixture, "chain.create", { name: "repo-check", repository: "not-a-forge-ref" }))
			expect(record(response.chain)).toMatchObject({ metadata: { bindings: { repository: "not-a-forge-ref" } } })
		} finally {
			await fixture.daemon.stop()
		}
	})
	...
	test("socket item.update rejects immutable selectors and daemon-owned fields", async () => {
		...
		try {
			...
			const invalidRequests = [
				...
				{ itemId, fields: { legacyItemNumber: 999 } },
...
`,
		...
		const daemon = await startCoderLoopDaemon({
			...
			scheduler: {
				...
				prompt: ({ item, runId }) => JSON.stringify({ itemId: item.id, legacyItemNumber: Number(item.itemId), runId, eventLog }),
				chainCompleteTriggerForChain: () => null,
			},
		})
		try {
...
`,
		...
		const daemon = await startCoderLoopDaemon({
			...
			scheduler: {
				...
				prompt: ({ item, runId }) => JSON.stringify({ itemId: item.id, legacyItemNumber: Number(item.itemId), runId, eventLog, sleepMs: 5_500 }),
				chainCompleteTriggerForChain: () => null,
			},
		})
		try {
...
`,
		...
		const daemon = await startCoderLoopDaemon({
			...
			scheduler: {
				...
				prompt: ({ item, runId }) => JSON.stringify({ itemId: item.id, legacyItemNumber: Number(item.itemId), runId, eventLog, sleepMs: 3_500 }),
				chainCompleteTriggerForChain: () => null,
			},
		})
		try {
	...
	test("queue unblock always resumes scheduler", async () => {
		const scenarios: readonly QueueUnblockOutcomeScenario[] = [
			{ kind: "success", preset: "loaded", targetStatus: "blocked", itemId: "target", dryRun: false },
			{ kind: "dry-run", preset: "loaded", targetStatus: "blocked", itemId: "target", dryRun: true },
			{ kind: "not-unblockable", preset: "loaded", targetStatus: "done", itemId: "target", dryRun: false },
			{ kind: "not-found", preset: "loaded", targetStatus: "blocked", itemId: "missing", dryRun: false },
			{ kind: "preset-load-error", preset: "missing", targetStatus: "blocked", itemId: "target", dryRun: false },
		]
		...
		for (const scenario of scenarios) {
			...
			try {
				...
				const responsePromise = sendDaemonRequest(fixture.socketPath, daemonRequest("queue.unblock", {
					chainName: fixture.chainName,
					itemId: scenario.itemId === "target" ? fixture.targetItemId : scenario.itemId,
...
const input = JSON.parse(prompt.split("\\n")[0] ?? prompt)
const writeLine = (line) => Bun.write(Bun.stdout, line + "\\n")
await appendFile(input.eventLog, JSON.stringify({ type: "start", itemId: input.itemId, legacyItemNumber: input.legacyItemNumber, runId: input.runId, phase: input.phase, cwd: process.cwd() }) + "\\n")
await new Promise((resolve) => setTimeout(resolve, input.sleepMs))
await appendFile(input.eventLog, JSON.stringify({ type: "end", itemId: input.itemId, legacyItemNumber: input.legacyItemNumber, runId: input.runId, phase: input.phase, cwd: process.cwd() }) + "\\n")
await writeLine("done:" + input.itemId + ":" + input.phase)
if (input.phase === "review") {
	await writeLine("PHASE DONE: issue=#" + input.legacyItemNumber + "; reason=phase-aware-runner review")
} else {
...
async function readItem(loopDataRoot: string, chainId: number, legacyItemNumber: number) {
	const store = openSqliteStateStore({ loopDataRoot })
	try {
		return store.getItemById(chainId, String(legacyItemNumber))
	} finally {
		store.close()
	}
}
