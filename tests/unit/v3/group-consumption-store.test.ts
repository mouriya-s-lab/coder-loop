...
async function runCase(mode: ConsumptionCase, databaseFile: string): Promise<{ readonly outcome: string; readonly durable: string }> {
	...
	const member: Task = { identity: memberId, group, input, dependsOn: [], priority: 0, state: { kind: "settled", settlement: memberSettlement, settledAt: 1 }, closure: { kind: "unallocated" } }
	const consumer: Task = {
		identity: consumerId,
		group: consumerGroup,
		input,
		dependsOn: [],
		priority: 0,
		state: mode === "consumer-ready" ? { kind: "ready" } : { kind: "settled", settlement: consumerSettlement, settledAt: 2 },
	...
	const snapshot: ObjectDomainSnapshot = {
		...
		groups: {
			[groupKey(group)]: { identity: group, members: [memberId], memberVersion: 1, wait: { kind: "none" }, consumer: { kind: "validator", definition, entrypoint: "root" }, state: { kind: "consuming", consumerTask: consumerId, consumerGroup, settlementsDigest: "digest", startedAt: 2 } },
			[groupKey(consumerGroup)]: { identity: consumerGroup, members: [consumerId], memberVersion: 1, wait: { kind: "none" }, consumer: { kind: "drain" }, state: { kind: "open" } },
		},
		awaits: {},
		admittedFacts: {},
	}
