...
export type ItemCommandArgs =
	| {
			action: "add"
			chainName: string
			// #419: opaque string item id (formerly `fixtureItemNumber: number`). #551 cleanly renames
			// the CLI flag to `--item` without a compatibility alias;
			// the parser now accepts opaque string ids via `parseRequiredItemId` instead of the
			// old numeric `parseRequiredFixtureItemNumber`. Only the discriminated-union field name was
			// renamed (`fixtureItemNumber` → `itemId`); no `--id` flag exists.
			itemId: string
			repoCwd: string
...
const chainCreateCliCommand = command({
	...
	args: {
		...
		configJson: option({ long: "config-json", type: optional(cmdString) }),
	...
	handler: (args): CliCommand => ({
		...
		args: {
			action: "create",
			name: args.name,
			configJson: parseOptionalJsonObjectFlag(args.configJson ?? null, "--config-json") ?? {},
...
const itemAddCliCommand = command({
	...
	args: {
		...
		issueFile: option({ long: "item-file", type: optional(cmdString) }),
...
async function runChainCommand(args: string[]): Promise<void> {
	...
	if (chainArgs.action === "create") {
		...
		// `--config-json` is the chain-bindings entry point. Repository is optional opaque business
		// data and remains in bindings; only baseBranch is extracted as engine mechanism state.
...
function formatItemListResult(result: JsonObject): string {
	...
	return items.map((raw) => {
		...
		// #419: render the opaque item id (formerly `fixtureItemNumber`). Daemon's `itemToJson` emits
...
function rootUsage(): string {
	return [
		...
		"  queue unblock <target> --item <item>",
		"  doctor <target>",
		"",
	].join("\n")
}
...
function statusItemSnapshot(item: ItemRecord, preset: Preset): StatusItemSnapshot {
	...
	// `fixtureItemNumber` engine column to back-fill from. The opaque `item.itemId` is the
...
function parseQueueUnblockMutationResponse(response: JsonObject, requestedItemId: string): QueueUnblockMutationOutcome {
	...
	const itemId = typeof itemIdValue === "string" ? itemIdValue : requestedItemId
	if (changed === false) {
...
export function parsePreset(value: BoundaryValue, presetDir: string): Preset {
	...
	// `item.fixtureItemNumber` to the resolver bypassing `[item.fields]`); with that escape gone the rule
...
// chain-create mechanism scalar baseBranch, normalizes a relative `presetPath` to absolute, and rejects nested
...
const CHAIN_CREATE_NON_BINDING_KEYS = new Set(["baseBranch"])
