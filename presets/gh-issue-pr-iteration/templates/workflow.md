...
- Queue/order/state: `CODER_LOOP_DATA_ROOT/state.db`
- Current issue handoff: `chains/<chain>/issues/<issue>.md` under `CODER_LOOP_DATA_ROOT`
- Shared durable facts: `chains/<chain>/shared.md` under `CODER_LOOP_DATA_ROOT`
- Runtime evidence: `chains/<chain>/evidence/` under `CODER_LOOP_DATA_ROOT`
...
- Do not stage runtime data, loop-data files, `.claude/scheduled_tasks.json`, runtime logs, or untracked runtime evidence into feature PRs.
...
Preserve the concrete recommendation order from the loop state store.
