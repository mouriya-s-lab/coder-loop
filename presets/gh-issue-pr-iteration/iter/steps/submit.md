...
- **Packet completeness** — fresh PR: the body carries the layered sections the target workflow demands, with commands/exits/excerpts, embedded screenshots, the E2E direct-run evidence, the runtime manifest, and the test-inventory delta line; the `coder-loop:current-state` index block is present with all five per-round arrays declared as `[]`. Retry: the delta comment carries the headline, the updated CandidateRef, one entry per **latest-round** feedback item with its evidence pointer, the **cross-round finding ledger** covering every row from iter-entry Step 2's history (one row per historical finding with `addressed / regressed-and-refixed / deferred #<issue>` + pointer), and the fresh-check table — a retry comment that re-narrates the full four-layer packet instead of the delta is a shape gap, as is one missing any latest-round feedback item, as is one whose cross-round ledger row count does not match iter-entry Step 2's ledger minus the latest-round subset. Every claim traces back to this run's verify and e2e reports. A secret value pasted into the PR body/comment is a hard gap — auth appears by resolution location only.
