...
describe("db-backed loop CLI", () => {
	test("status task tree projects persisted recursive identity without flat item synthesis", async () => {
		...
		try {
			...
			store.setCurrentRun({ chainId: chain.id, phase: "iteration", runId: "persisted-run-one", startedAt: 1_800_000_200, extra: storedItemExtra({}) })
			store.setCurrentRun({ chainId: chain.id, phase: "iteration", runId: "persisted-run-two", startedAt: 1_800_000_201, extra: storedItemExtra({}) })
