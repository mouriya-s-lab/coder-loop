...
- **closure**（单 session）：重读 live state 做 drift 检查（sameness 路由回产出该 artifact 的 phase），按序执行 merge / close effect，确认 live terminal state 后最后写 `done` / `moot`。终局状态只由它写；`done` 落地同时是跨仓解锁的触发点——引擎自动把 `dependsOn` 指向本 item 的 blocked item 恢复为 actionable，closure 不做手动 unblock。
- **blocked-responder**：跨仓 unblock 的最小化 responder——在 blocker 仓创建 follow-up issue、经 `item add` 注入 blocker 仓的 chain（`createItems` 授权）、对自己的 blocked item 写 `dependsOn`（`item.dependsOn` 特权授权），解锁本身交给引擎的依赖自动恢复。
