...
> **SUPERSEDED（部分）**：裁决 1 的粒度条款（任务 = 同一 (item, phase) attempt 链）被 2026-07-31 三域模型裁决 supersede——task 身份是引擎 id，不由值组合定义，(item, phase) 降为绑定元数据；依据即本文 §6-4 方法论（verbatim 未定义粒度，(item, phase) 是会话代答的解释层）。登记见 RFC #546 收敛稿 §9-1。其余条款（对外无状态、递出面定理、系统自证完备）存活。
>
