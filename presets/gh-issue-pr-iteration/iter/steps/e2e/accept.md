...
The report must contain: `E2E run` with surface, setup (auth by resolution location), the real entry driven, and observed behavior; `Browser acceptance rows` covering every browser-Env row you named in the dispatch's `Step focus` (the set you enumerated from the issue's acceptance tables; when the verify report is already in hand, cross-check that its `deferred: e2e step` rows are the same set — a mismatch is a gap on whichever report dropped a row); `Runtime manifest` including the standing environment; `Artifacts` mapped to what each proves; `Problems`. Structurally incomplete → send back the missing fields before judging substance.
...
- **Deferred rows closed** — every browser-Env row from the dispatch's `Step focus` appears here with an observed-vs-Expect verdict from the real UI walk (and once the verify report arrives, its deferred set must be covered too — same rows by construction). A failing row is a product failure: route it back to implementation, then re-dispatch verify and e2e in parallel for the full contract. A deferred row absent from this report leaves the contract unverified — send it back.
