...
2. The issue's linked PR → the retry instruction source and the branch-continuity input. Resolution order: the bound `ISSUE_PR` when set; otherwise the structural closing-keyword linkage — split `{{REPO}}` into owner/name and run:
   ...
   From the body, resolve the `coder-loop:candidate-ref` block and the `coder-loop:current-state` index per `{{PRESET_ROOT}}/common/packets.md`. Head-only pointers you need this round: `verificationPacketUrls[length-1]` (the latest packet — failing rows in the current candidate) and `contractMarkerUrl` (the marker you must satisfy). Also fetch any PR comments posted **after** the newest verdict's timestamp (operator additions — filter the comment listing by `createdAt > reviewVerdictUrls[length-1].createdAt`); these are additive corrections that shared.md's verdict block cannot capture in advance.

   **Cross-round history** — the full sweep of `reviewVerdictUrls` / `diffAuditReportUrls` / `verificationAuditReportUrls` — is retry input, not fresh-round input. Read it only when this run is a Retry (Step 1 classification). Even then, the primary retry instruction is not an array scan: it is the round-opening verdict block written into `{{SHARED_CONTEXT_FILE}}` by the phase that bounced this round (see the next input item). Fall back to a full array scan only when the shared verdict block is absent / unparsable, or when a shared finding cites a historical URL you must chase to confirm a regression.
...
4. `{{SHARED_CONTEXT_FILE}}` → your primary Retry instruction source. Grep the current issue section `## Issue #{{ISSUE}}` and locate the highest-numbered `### Round <n>` block. If that round opens with a `<!-- coder-loop:shared:verdict source=… -->` block (any source: review / diff-audit / verification-audit / verification / publish), **that block is the authoritative retry instruction** — its findings list is what this Retry must address, with the exact anchors it names. Any historical-URL chase (regression check on a prior finding the shared verdict block cites) fetches only the specific URLs the block names, not the full array. If `## Issue #{{ISSUE}}` is absent or its latest round has no verdict block (Round 1 opened by the initial iteration entry itself), this run is Fresh and shared.md carries only context; the primary source is the current executable-contract marker read in item 1. If `## Issue #{{ISSUE}}` is present but truncated / corrupted, fall back to the full array scan described in item 2.
5. The queue's selected item — `coder-loop status {{TARGET_CWD}} --json` → `queue.selected` — must match {{ISSUE}}. Mismatch, or the status read itself failing → record the infrastructure failure and jump to Step 5 (wrap-up); do not improvise a different issue.
...
- **Fresh (no `## Issue #{{ISSUE}}` section yet)**: create the section, open `### Round 1 — opened by iteration as fresh` with its `<!-- coder-loop:shared:issue … -->` and `<!-- coder-loop:shared:round round=1 … -->` markers (no verdict block: Round 1 has none), then append your `#### Phase: iteration` note with its `<!-- coder-loop:shared:phase-note phase=iteration … -->` marker.
- **Retry (round already open, opened by an upstream retry / drift verdict)**: append the `#### Phase: iteration` note under the current round; if a prior iteration note exists for the same round (partial run recovered), overwrite that note in place instead of stacking a duplicate.
- **`contract_invalid` exit (only iteration retry-writer branch)**: compact per the state-contract rule — close the current round with a `### Round <n> — closed by iteration as contract_invalid at {{RUN_ID}}` summary, open `### Round <n+1>` with a `<!-- coder-loop:shared:verdict source=iteration status=contract_invalid run={{RUN_ID}} -->` block naming the contract defect + evidence comment URL, and only then append your `#### Phase: iteration` note if any (usually the note is folded into the verdict block on this branch).

Note contents (concise, source-cited): run ID; spawn classification; the final task list with each line's outcome; files changed; the typed runtime manifest (`durable` or `recreatable`) from the e2e step; artifacts; PR number / URL or comment URL plus the CandidateRef identity; blockers / unresolved risks; proposed child issue specs when scope was incomplete. If `{{CURRENT_ISSUE_FILE}}` exists, issue-local detail may go there instead of the shared note (only the pointer stays in shared).
