...
- **Local integration suites**: `bun run test:integration`（依次运行 `tests/integration/cli/`、`scheduler/`、`daemon/`，每个目录内的 `*.integration.ts` 逐文件串行）
- **All local test layers**: `bun run test:all`（按 unit → integration-cli → integration-scheduler → integration-daemon fail-fast）
- **Single/background batch**: `bun scripts/run-tests.ts --batch <name>`；追加 `--background` 后用 `bun scripts/run-tests.ts --status [runId]` 查看 `.test-runs/` 中的进度。目录准入规则见 `docs/test-boundaries.md`。
