...
Write the actionable issues into `{{TARGET_CWD}}/central SQLite state DB` as queue items. Write per-issue handoff files into `runtime/issues/`. Validate the resulting runtime with `--check-runtime`.

## Inputs

- Issue numbers and metadata from `plan/create-issues` (only `issues_created` verdict reaches this fragment).
- Target's existing `central state DB` (planning may be a fresh init OR a top-up onto an existing queue).
- `contract.md` §1 (`item` fields).

## Procedure

1. Read existing `{{TARGET_CWD}}/central SQLite state DB`. Preserve existing `queue`, `repository`, `baseBranch`, `recentRuns`, `current` fields if present.
...
3. **Construct queue items**. Each item is:
   ```json
   {
     ...
     "pr": null,
     "lastRunId": null,
     "issueFile": "loop-data/chains/<chain>/issues/<N>.md",
     "evidenceDir": "loop-data/chains/<chain>/evidence/issue-<N>",
     "agentCwd": null
   }
   ```
...
5. **Write per-issue handoff files** at `{{TARGET_CWD}}/loop-data/chains/<chain>/issues/<N>.md`. This file is what `iter/read-context` reads for issue scope context. Minimum content:
...
6. **Create evidence directories**: `mkdir -p {{TARGET_CWD}}/loop-data/chains/<chain>/evidence/issue-<N>` for each queued issue. Empty directory at this stage; iter agent will populate.

7. **Update `central state DB`**:
...
8. **Run schema check**:
   ...
   ```
   Exit 0 required. Any error → unwind your edits to `central state DB` (restore the backup you made in step 1) and bail with `queue_init_failed`.

## Failure handling

If `--check-runtime` fails after your edits:

- restore `central state DB` from the snapshot taken before edits;
