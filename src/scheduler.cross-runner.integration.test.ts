...
function createChain(
	...
	// Preset-owned values flow through metadata.bindings rather than first-class chain columns.
	const resolvedMetadata = metadata !== undefined && Object.hasOwn(metadata, "bindings")
		? metadata
		: { ...(metadata ?? {}), bindings: { projectId: 309, projectRepo: "mouriya-s-lab/coder-loop" } }
...
function createItem(store: ReturnType<typeof openSqliteStateStore>, chain: ChainRecord, legacyItemNumber: number) {
	return store.createItem({
		chainId: chain.id,
		itemId: String(legacyItemNumber),
		repoCwd: REPO_ROOT,
		status: runtimeStatus("queued"),
		attempts: 0,
		title: `issue ${legacyItemNumber}`,
		extra: storedItemExtra({}),
		createdAt: 1_800_316_001 + legacyItemNumber,
		updatedAt: 1_800_316_001 + legacyItemNumber,
	})
}
