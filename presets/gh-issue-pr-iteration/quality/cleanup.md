...
Side-effect discipline — each phase declares every side effect as it works, then sweeps its own ledger before exit.

## Declare every side effect
...
- Stop what you started when your step no longer needs it — with one deliberate exception: the deliverable's e2e runtime stays **up**, documented in the runtime manifest (PIDs, ports, logs, stop commands), because the verification phase executes against it and owns its teardown. Anything else you intentionally leave running for a later step, say so explicitly.
...
## Sweep the ledger before exit

- Keep a side-effect ledger across your own steps: every process started, temp path written, branch/worktree created — recorded as each step reports it.
- Ownership follows the runtime handoff ADT. **Iteration**: for `durable`, leave only the supervisor-owned runtime; for `recreatable`, stop every phase-owned process and preserve the pinned worktree/manifest. **Verification**: execute by kind, stop any runtime it started, and use the durable manifest's stop command when final teardown owns that environment (the packet's `runtime.cleanup` records what happened). **Read-mostly phases (diff-audit / verification-audit / review)**: stop nothing they did not start; they never re-drive the e2e runtime.
