...
> 重编原型：`v3-issue/A.md`（原始标题：`# v3 RFC A 文档合集`）。
>
> 本文以 A.md 的八篇正文、来源标注、旧编号注释、表格与既有 Mermaid 图为材料全集，按依赖拓扑重排篇章，并把原文中尚未闭合的候选逐项放回当前设计检验。能保持公共不变量的候选直接纳入；存在多个同样合法语义的候选改写为声明 ADT；需要旁路权威、双重事实源或破坏单调性的候选予以拒绝。正文内原有的 `A.md` 交叉引用继续用于来源追溯，在 B.md 中按同名篇章理解其逻辑位置。
...
```mermaid
flowchart LR
    M0[第0篇 公共词汇与公理] --> M1[面1 定义态]
    M1 -->|pinned definition| M2[面2 函数域运行时]
    M2 -->|returned value 或 exception| M3[面3 对象域]
    M4[面4 入站边界] -->|typed admit| M3
    M2 -->|固化调用| M5[面5 出站边界]
    M5 -->|封闭 provider 事实| M3
    M0 --> M6[面6 hook]
    M2 --> M6
    M3 --> M6
    M1 --> M7[面7 GUI]
...
```mermaid
flowchart TB
    D[定义态 静态声明] --> C[编译态 CompileEnvelope]
    C --> P[发布态 immutable bundle]
    P --> I[实例态 exact pin]
    I --> F[函数域闭包求值]
    F --> O[对象域 committed transition]
    F -. context与副作用投影 .-> V[观测态]
    O -. 持久事实投影 .-> V
    V -. 不拥有推进权 .-> O
```
...
以下篇章按 owner 依赖排列：先读公共公理，再读定义与执行本体，随后读对象域和双向边界，最后读 hook 与 GUI 两个投影面。旧 RFC 编号注释随所属篇章保留，仅承担来源映射，不再决定阅读顺序。

## 设计插入判定规则

来源标签只回答一段内容从哪里来，不替代当前设计对它的兼容性检验。旧 RFC 候选和主 session 推导都必须放回公共模型：若能由既有不变量唯一推出，就直接成为当前设计；若多个行为都满足不变量，就把差异建模为声明参数；若必须增加旁路权威、第二条跨域通道、可回退历史或无法穷尽的万能动作，就拒绝该候选，而不是把判断转交给 operator。

```mermaid
flowchart TB
    C[候选机制] --> I{能否保持唯一来源 针眼通道 前向单调 封闭ADT}
    I -->|唯一兼容形态| A[直接纳入当前设计]
    I -->|多个形态均兼容| P[建模为声明参数或ADT variant]
    I -->|需要例外或旁路权威| R[拒绝候选并使用既有机制]
    A --> N[写入规范正文与验收]
    P --> N
    R --> N
```
...
| 原候选 | 插入判定 | 当前设计中的落点 |
|---|---|---|
| `preset` 同时表示工作流与 task 内部单位 | 拒绝同名双粒度 | `preset` 保留完整工作流及发布 bundle；内部单位为 `step`，路由值为 `nextStep` |
| fail 完整配置 | 纳入并拆正交轴 | retry policy 与 exhausted action 分离；面 3 拥有跨 task policy |
| 同时态 map 可见性与顺序 | 纳入唯一兼容形态 | 入口快照、互不可见、无声明顺序、all-settled barrier |
| waiting window 重置或固定 | 两者均合法 | `fixed-deadline | sliding-deadline` 由定义声明 |
| await 在现场丢失后恢复同一 continuation | 拒绝 | 现场存在则 resume；现场丢失按 `active loss → exception` 落定 |
| `admit` owner | 纳入 | 面 3 拥有协议；面 2 与面 4 只提供内部、外部 adapter |
| 特殊 forward decision | 拒绝 | 使用普通 finalizer/correction task 与 typed admit，不增加隐藏推进动词 |
| “重试/放弃重试”同枚举 | 拒绝混轴 | retry policy 决定是否重试；exhausted action 决定随后 skip 或 stop |
| provider fact 消费映射 | 纳入 | 面 5 生产封闭事实，面 3 按 variant 唯一消费 |
| 五个命令名作为封闭领域代数 | 拒绝过强枚举 | 规范封闭 transition family；`commit` 仅是原子事务边界 |
| committed transition hook anchor | 纳入 | 作为既存、稳定、只读的对象域事实锚点 |

后文的“已纳入”表示通过上述插入检验，不声称它曾由 operator 逐字裁决；“已拒绝”表示候选与当前设计冲突。实现不得重新打开这些结论，也不得把 provenance 强度误作产品决策队列。
...
```mermaid
flowchart LR
    A[AI agent effect体] --> W[副作用世界]
    W --> S[self-report]
    W --> M[measurement]
    W --> X[未提升副作用]
    S -->|unknown 经 parser 成为 T| C[typed context]
    M -->|map 成为 Just T| C
    X -->|本次交接语义丢弃| D[文件 日志 进程 Git残迹]
    D -. 以后如需消费必须重新采样 .-> M
```
...
```mermaid
flowchart LR
    O1[对象域 item死值] -->|入口 context0| F[函数域闭包 五时态]
    F -->|returned value| O2[对象域 committed transition]
    F -->|exception| O2
    F -. context1 context2 context3与内部驳回不进入任务账本 .-> O2
```
...
```mermaid
flowchart TB
    C[完整 context3] --> P{全部谓词为 true}
    P -->|是| N{合法后继数量 n}
    N -->|n大于等于2| CH[消费 agent或map chooser]
    N -->|n等于1| EN[引擎填唯一后继]
    N -->|n等于0| UP[升层到 task或chain]
    P -->|否| F{显式 fail}
    F -->|有| FS[执行 fail步骤]
    F -->|无 NIL| CFG[步骤到任务到组到item配置级联]
    CFG --> STOP[配置穷尽后全局停机]
```
...
```mermaid
flowchart LR
    SH[step交接合同] --> G[交付值包 解析测量 谓词 路由]
    TH[task交接合同] --> G
    SH -->|attempt级 可蒸发重放| SF[函数域现场]
    TH -->|committed且不可撤销| TL[对象域账本]
    SF -->|只折叠为二值| TL
```
...
来源：record-3 第 14 轮操作员原话与该轮异常消费者收敛；完整配置由面 3 §9.1 按步骤、任务、群组三个正交层级闭合。
...
产值与检查是两个正交的面。每个 context 值都有一个谓词槽；默认谓词恒为 true，因此没有显式检查时无需生成空代码。显式谓词只在时态五读取完整 context 并返回 true 或 false。检查是否通过不决定下一个 step 是谁；路由值已经存在也不能绕过检查。任一检查失败时，正常路由值不被消费，执行转入约定的 fail 路径。

`nextStep` 是特殊 context 值，因为引擎可以在封闭候选集足够小时接管填值。当声明允许的后继数量 `n ≥ 2`，agent 或脚本必须从合法 step identity 闭集中选择；`n = 1` 时引擎直接填入唯一值；`n = 0` 时发生升层，进入下一个 task 或 chain 结束。

`preset` 继续表示完整工作流及其发布 bundle；task 内部的步骤级定义单元统一称为 `step`。二者拥有不同 identity、编译边界与路由粒度，复用同一名称会破坏定义引用和运行时诊断的唯一性，因此原“步骤级 preset”候选不进入当前设计。

fail 是一个特殊 step，而不是引擎硬编码的业务失败枚举。preset 可以显式声明由哪个 fail step 消费检查失败或程序异常，也可以完全不声明。未声明时由专用 NIL 值补位，依次查找 operator 全局配置中“步骤→任务→组→item”的级联策略；若配置仍未提供消费者，NIL 硬默认的最终形态是面 3 执行的全局停机动作。这个优先级使 fail 在类型追踪中始终有值，不出现 `undefined`。配置 ADT 由面 3 §9.1 拥有，并把 retry policy 与 retries exhausted 后的 action 分成正交字段。
...
- **路由面**：在检查通过后决定 `nextStep` 或升层的正交流程；`nextStep` 是引擎可接管的特殊 context 值。
...
```mermaid
flowchart LR
    E[CompileEnvelope identity] -->|包含判定与findings| C[compiled product identity]
    C -->|发布完整资产| D[definition content identity]
    E -. finding规则变化可独立变化 .-> C
    C -. normalized语义变化 .-> D
```

### compile、publish、pin 与 resolve

```mermaid
sequenceDiagram
    participant A as Preset Author
    participant C as Compiler
    participant P as Publisher
    participant I as Instance
    participant R as Resolver
    A->>C: 类型 prompt map与交接声明
    C-->>A: CompileEnvelope
    C->>P: strict closed compiled product
    P-->>P: staging fsync digest rename
    P-->>I: live definition ref
    I->>I: pin exact ref
    I->>R: spawn或resume时resolve
    R-->>I: verified immutable bundle或typed resolution failure
```

### typed admission 与定义失败分域

```mermaid
flowchart TB
    U[外部 unknown candidate] --> P[pinned parser]
    P -->|accepted| T[typed item值]
    P -->|rejected| AR[admission rejected]
    S[声明文本闭包] -->|结构矛盾或未闭合| CR[compile rejected]
    B[pinned bundle] -->|缺失或digest不符| DC[definition corrupt]
    L[pre-ref历史] -->|无真实artifact证据| LU[legacy definition unproven]
```
...
同一时态内多个 map 使用唯一兼容的落定语义：map 只能读取该时态入口处已经存在的值，不能读取同时态 sibling 的产出，因此彼此不可见，也不存在声明顺序。
**同时态 map 的落定与 barrier。** 同一前置或后置 map 时态中的 map 集合由 pinned definition 固定。所有 map 读取该时态入口处的同一份只读 context snapshot；运行时可以采用任意顺序与并行度，但 sibling 结局不得触发取消或跳过。每个 map 最终落为 `produced(T) | absent | fault(MapFault)`。`produced(T)` 在 barrier 前只是待提交值；barrier 收齐全部结局后，先聚合 fault，再按 compiled contract 判定必需值缺席。存在 fault 或必需值缺席时，整个 batch 只产生一次聚合的程序面异常，不发布部分 context；仅当 batch 合法时，才按唯一值名原子形成 `context₁` 或 `context₃`。不提供 preset 级 fail-fast 参数。closure 级 stop、deadline 与进程丢失属于外部中断，不是 sibling 失败传播。前置与后置 map 对称适用本规则。复用代价由观测单元级 product value 消解，不由 map 间可见性补偿。【设计插入判定 | 五时态可见性与无取消传播 | 已纳入】
...
sequenceDiagram
    ...
    E->>M: 时态一 前置map读取context0
    M-->>E: context1或程序异常
    E->>E: 时态二 组装并固化prompt
    E->>A: 时态三 启动agent
    loop 直到必写值全部accepted
      A->>C: 提交unknown payload
      C-->>A: rejected错误或accepted
    end
    E->>M: 时态四 后置map读取context2
    M-->>E: context3或程序异常
    E->>E: 时态五 纯谓词与路由
    E-->>E: returned value或exception
```
...
flowchart TB
    ...
    N -->|多个| M[读取声明chooser]
    N -->|一个| E[引擎自动填]
    N -->|零个| R[封装returned value越过针眼]
```

### fail、NIL 与 typed escalation

```mermaid
flowchart TB
    X[谓词false或可路由程序异常] --> F{显式fail后继}
    F -->|有| S[步骤内处理]
    F -->|无| N[NIL查询operator配置]
    S -->|仍在函数域可完成| Z[重试步骤 跳过或结束路径]
    S -->|需要跨task动作| E[exception携带typed escalation]
    N -->|配置给出跨层动作| E
    N -->|级联穷尽| E
    E --> O[面3 evaluator产出对象域action]
```
...
sequenceDiagram
    participant E as Engine
    participant M as Map runtime
    E->>M: 前置map context0到context1
    E->>E: prompt projection为空并固化
    E->>E: 关闭空agent时态 context2等于context1
    E->>M: 后置map context2到context3
    E->>E: 纯谓词与正常或fail路由
```
...
```mermaid
flowchart LR
    C1[当前闭包 typed context] --> R[returned value]
    R --> T[面3 committed transition]
    T --> I[后继item值]
    I --> C2[下一闭包 context0]
    O[stdout 共享文件 session正文] -. 未提升不得成为输入 .-> C2
```
...
同一时态多个前置 map 读取入口快照、互不可见，并以 all-settled barrier 原子形成下一份 context；执行器可以串行或并行，但不得把物理执行顺序暴露为定义语义。前置与后置 map 对称遵守面 1 §2。
...
运行时先穷尽求值谓词，再决定是否读取正常路由；因此，即使 `nextStep` 已经由某个来源形成，任一谓词为 false 时该值仍不被消费。

`nextStep` 是特殊 context 值，其特殊性只在于引擎可以按 pinned 后继闭集接管填值：
...
NIL 触发对 operator 全局配置级联的查询；如果权威结果仍为空，闭包侧最终只交付携带“级联穷尽” escalation 的 `exception`。面 3 evaluator 消费该 escalation 后产出全局停机 action，由执行器作为最高层级对象域动作执行。
...
|---|---|---|---|
| 时态一：map 启动失败、超时、抛错、输出无法提升或必需值为 `Nothing` | 程序面 | 执行显式程序异常路由；未声明时进入 NIL 查询 | 情况 A：级联给出对象域动作（如任务级放弃）时，发出 typed escalation（结构化内容，走 `exception` 分支，不构成第三种出口）；情况 B：级联穷尽、无任何消费者时，闭包侧交付携带“级联穷尽” escalation 的 `exception`，面 3 evaluator 消费后产出全局停机 action，由执行器作为最高层级对象域动作执行；其不可被 par 边界吸收的语义由该动作的对象域层级保证，而非由特殊 exception 类型保证 |
...
| 时态四：map 启动失败、超时、抛错、输出无法提升或必需值为 `Nothing` | 程序面 | 执行显式程序异常路由；未声明时进入 NIL 查询 | 情况 A：级联给出对象域动作（如任务级放弃）时，发出 typed escalation（结构化内容，走 `exception` 分支，不构成第三种出口）；情况 B：级联穷尽、无任何消费者时，闭包侧交付携带“级联穷尽” escalation 的 `exception`，面 3 evaluator 消费后产出全局停机 action，由执行器作为最高层级对象域动作执行；其不可被 par 边界吸收的语义由该动作的对象域层级保证，而非由特殊 exception 类型保证 |
| 时态五：任一谓词为 false | 声明的 fail 路由或 NIL | 不消费正常路由，执行 fail 步骤或步骤级默认动作 | 情况 A：级联给出对象域动作时，发出 typed escalation（结构化内容，走 `exception` 分支，不构成第三种出口）；情况 B：级联穷尽、无任何消费者时，闭包侧交付携带“级联穷尽” escalation 的 `exception`，面 3 evaluator 消费后产出全局停机 action，由执行器作为最高层级对象域动作执行；其不可被 par 边界吸收的语义由该动作的对象域层级保证，而非由特殊 exception 类型保证 |
...
flowchart TB
    ...
    G --> C[C]
    G --> D[D 动态admit]
    A -->|returned后才物化| A2[A2]
    B -->|await派生| BC[B-check]
    BC -->|值回注同一闭包| B
    C -->|attempts耗尽| CE[exception落定]
    A2 --> J[固定join]
    B --> J
    CE --> J
    D --> J
...
flowchart TB
    ...
    P --> C
    C -->|不关心值| D[drain]
    C -->|需要判断| V[validator或finalizer task]
```
...
```mermaid
sequenceDiagram
    participant B as Task B closure
    participant O as Object domain
    participant C as B-check closure
    B->>O: await并派生B-check
    O->>B: 释放运行锁 保存closure现场
    O->>C: 建立独立task与锁
    C-->>O: returned value或exception
    O->>O: committed transition只消费一次
    O-->>B: 回注typed结局并重取锁
    B->>B: 从同一函数实例继续
```

### typed admit 同时判位置与时机

```mermaid
flowchart TB
    P[提议 新task加位置与时机与事实identity] --> A[admit]
    A --> C{pinned合同 开放前沿 结束事实 授权}
    C -->|全部成立| OK[提交task identity与位置 消耗幂等键]
    C -->|位置失效| PR[position unavailable 加开放前沿]
    C -->|时机无效| TR[timing invalid]
    C -->|合同拒绝| CR[contract rejected]
    C -->|越权| UR[unauthorized]
    PR --> NP[提议方可换位置重新声称]
```

### 业务返回、exception 与 escalation

```mermaid
flowchart LR
    R[合法业务variant] --> D[派发表显式处理]
    X[runner崩溃或无返回] --> E[exception落定]
    E --> G[最近群组边界隔离或声明消费者]
    S[函数域typed escalation] --> P[面3 policy evaluator]
    P --> A[穷尽对象域action]
    A --> C[committed transition]
```

### task、closure 与 GC 生命周期

```mermaid
stateDiagram-v2
    [*] --> Ready: task物化
    Ready --> Running: 获得锁并建立closure
    Running --> Suspended: await并释放锁
...
sequenceDiagram
    ...
    participant O as Open frontier
    S->>L: 验证当前task锁所有权
    S->>H: 原子记录交付与task落定
    S->>O: 同事务物化后继或消费群组
    alt 任意提交前崩溃
...
来源：沿用旧 546 §1，并通过公共模型的对象域/函数域边界检验。【旧 RFC 候选 | A.md 旧 546 §1 | 已纳入】
...
来源：沿用旧 546 §2，并删除已经失效的外部证据行号引用；其问题诊断与当前双域模型一致。【旧 RFC 候选 | A.md 旧 546 §2 | 已纳入】
...
来源：沿用旧 546 §3，并以总纲的对象域、函数域、值与针眼词汇收束。【record 收敛 | record-1 assistant 评价段 | 未反驳】【旧 RFC 候选 | A.md 旧 546 §3 | 已纳入】
...
来源：沿用旧 546 §4；冻结机制只消费面 1 第 5—7 节的合同，符合定义不热改运行实例的不变量。【旧 RFC 候选 | A.md 旧 546 §4 | 已纳入】
...
来源：沿用旧 546 §5，并以总纲 0.3 的运行时生长边界收束；这里的分阶段供参不要求引入额外函数式运行时。【旧 RFC 候选 | A.md 旧 546 §5 | 已纳入】
...
等待策略是定义参数而不是外部裁决。`WaitWindow = none | { duration, mode }`，其中 `mode = fixed-deadline | sliding-deadline`。`fixed-deadline` 从群组第一次全体落定时建立不可移动截止点；窗口内新增成员后，若截止点已经过去，则在新成员全部落定后立即结束。`sliding-deadline` 在每次成员增长并再次全体落定后重新建立完整时长。两种模式都只影响结束事件何时可提交，不改变“结束前可 admit、结束后不可重开”的单调边界。

来源：seq 退化与统一消费关系来自 record-1 2:30—2:35【record 收敛 | record-1 2:30—2:35 | 未反驳】；等待窗口与期满日志来自 record-2 3:06—3:07【操作员原话 | record-2 3:06 | 已裁决】【record 收敛 | record-2 3:07 | 未反驳】；固定与滑动两种行为按 record-2 3:07 的“均自洽”结论建模为声明 ADT。【设计插入判定 | 当前单调结束模型 | 已纳入】

## 7. await 与 dependsOn 不是两种隐藏队列

B 需要 B-check 时，B 的闭包尚未返回。await 允许运行中的 task 派生下级 task，持久记录 `AwaitId(parentTask, parentAttempt, awaitSite)`，保存自己的 continuation 资源，释放执行锁，等下级返回后把值送回同一个函数实例继续执行。B-check 仍是对象域中的 task，有自己的闭包、资源和锁；它不是 B session 里一个无法观察的子进程。

如果没有 await，开发者通常会用 resume prompt、共享文件或手写 status 把 B-check 结果塞回 B。这些旁路既绕开类型检查，也无法证明 crash 后值被消费了几次。await record 把等待关系、child identity 与结果消费 token 放进对象域账本；continuation 本身仍属于 closure 私有资源，不上浮为可序列化的函数域状态。B-check exception 不被伪造成业务值，其处理仍由声明的消费者结构决定。
...
来源：await/dependsOn 的对象域关系沿用旧 546 §7；dependsOn 与群组消费的区别由 record-1 2:30 澄清。【操作员原话 | record-1 2:30 | 已裁决】 continuation 存续时的恢复由 durable AwaitId 与消费 token 闭合；continuation 丢失后的同 attempt 重放候选违反函数域不持久化和面 5 active-loss 合同，予以拒绝。【设计插入判定 | 针眼边界与 provider 事实 ADT | 已拒绝】
...
来源：位置与时机来自 record-1 2:48【操作员原话 | record-1 2:48 | 已裁决】；重新声称与只判不选来自 record-2 3:09—3:10【操作员原话 | record-2 3:09 | 已裁决】【record 收敛 | record-2 3:10 | 未反驳】；同端口两类调用者来自 record-1 2:28【record 收敛 | record-1 2:28 | 未反驳】。owner 按副作用边界唯一落定：面 3 拥有 admit 协议与 committed transition，面 2/面 4 分别拥有内部/外部 adapter。【设计插入判定 | 单一 admission 端口 | 已纳入】
...
对象域不提供特殊 forward decision 或 override-advance。该候选会绕过普通返回的 typed consumer，为同一汇合点增加第二个推进权威，也使“封闭命令集”失真。判定 task 或 finalizer 坏死时，结果按 `exception` 与声明 policy 落定；需要继续时创建普通 correction/finalizer task，经同一个 typed admit 与 committed transition 前向推进。operator 的 daemon、unblock、暂停/恢复和重排命令只处理运维状态，不伪造业务返回或替 join 选择分支。

用户看到的 exception 应包含 attempt 和 closure identity，并与业务返回 tag 分栏；没有消费者时界面要显示“结构在此停止”，而非永远 spinning。验证至少包括业务负面返回派生 correction、进程崩溃不产生 correction、群组隔离 exception、无消费者时开放前沿停止，以及任何运维命令都不能绕过 typed consumer 提交业务推进。

来源：异常与业务状态分离沿用旧 546 §9，并以面 2 的针眼出口校准词汇；旧 §9 的特殊 forward decision 与单一推进权威冲突，未纳入当前设计。【设计插入判定 | 针眼通道与封闭推进权威 | 已拒绝】

## 9.1 typed escalation 与 fail 级联的 policy owner

级联配置由本面拥有一个显式 schema，而不是若干 optional flag。步骤异常首先读取 `RetryPolicy = never | limited { maxAttempts, backoff }`；只有策略为 `never` 或 attempts 已耗尽时，才读取 `StepExhaustedAction = skip-step | stop-task`。这样“是否重试”与“不再重试后怎么办”是两个正交轴，原“重试/放弃重试”不再作为同一个含混枚举。`skip-step` 只有在 compiled consumption graph 证明后继不要求该 step 缺失的产出时才是合法配置，否则定义态直接产生 finding；`stop-task` 则以 exception 结束 task。任务处置为 `skip-task | stop-group`，组处置为 `advance-next-item | stay-on-current-item`。preset 显式 fail 路由优先，其次由 evaluator 按配置逐层选择，最终没有消费者时进入硬默认。

面 2 执行 retry 与 `skip-step`；`stop-task` 越过 task 边界时，只在 `exception` 分支携带 typed escalation。本面的 evaluator 以该信号和 policy 为输入，纯求值出穷尽的 task/group/item action ADT，不直接产生副作用；执行器再落实跨层动作，并把结果写成 committed transition。两者都不重跑面 2 的异常归属判断，也不读取闭包内部步骤状态。

级联顺序固定为：步骤异常先应用 retry policy；不再重试时选择跳过步骤或停止任务；任务停止再选择跳过任务或停止群组；群组停止最后决定是否推进到下一个 item。evaluator 必须对 policy ADT 穷尽，未知 variant 不得落入 default。配置与显式 fail 路由都穷尽时，evaluator 产出全局停机 action，由执行器作为本面最高层级对象域动作执行。该停机不是一种绕过针眼的特殊 exception；其不可被普通 par 边界吸收的语义由动作的对象域层级保证，与面 2 第 6 节执行表保持一致。

来源：级联层级来自 record-3 第 10 轮【操作员原话 | record-3 第 10 轮 | 已裁决】；原含混表述通过拆分 retry policy 与 exhausted action 消解，不需要在相互重叠的词义间选择。schema/evaluator 归面 3，步骤内执行归面 2，跨 task/group/item 动作归面 3。【设计插入判定 | 时态副作用边界与 ADT 正交性 | 已纳入】
...
来源：逐 variant 映射来自 division-plan.md“面 5”，并通过“producer 只产封闭事实、consumer 按 variant 唯一穷尽”的边界检验。【设计插入判定 | 面 5 producer / 面 3 consumer | 已纳入】

## 10. 五类事实事件和锁把 crash 变成可重放问题

当前模型封闭的是五类 transition family，而不是五个表面命令名：

1. **task admission**：应用 pinned definition，并以 typed admit 物化 task；
2. **lease lifecycle**：取得或释放 task 的唯一运行权；
3. **task settlement**：提交 `returned(value) | exception`；
4. **await suspension/resumption**：记录 AwaitId、child identity 与一次性结果消费；
5. **group termination/consumption**：记录 waiting、期满或立即结束，并把完整落定向量交给固定 consumer。

`spawn`、`admit`、`release`、`await` 可以继续作为 CLI 或内部 command 名称；`commit` 只表示上述 transition 的原子持久化边界，不能充当允许任意 mutation 的万能领域动词。可选等待期满是 group termination 的 typed 原因和时间证据，重放不重新等待。

锁表回答哪个 run 当前拥有 task 的执行权，并保证每个 task 至多一个活 run。锁不能替代 settlement 与 group 事实，事实也不能替代 lease：只有 current row 会在返回与派生之间留下不明中间态，只有事件却没有执行租约则允许两个 scheduler 同时启动同一 task。

一次 committed transition 原子验证相应 family 的前置条件、写入其封闭事实，并更新同一事务内的 lease、后继 application 或群组消费。状态、GUI 与文本日志只是 durable history 的具名投影，不能反向拥有 mutation 权。fault injection 必须覆盖 admission、lease、settlement、await suspension/resumption 与 group termination/consumption，重启后得到同一开放前沿且没有双跑。
...
来源：沿用旧 546 §12 的 GC、publication 与 residue 合同；先冻结证据再回收资源保持历史单调。【旧 RFC 候选 | A.md 旧 546 §12 | 已纳入】
...
来源：沿用旧 546 §13；finalizer 作为普通 typed task 消除了 daemon 业务特判。【旧 RFC 候选 | A.md 旧 546 §13 | 已纳入】
...
来源：沿用旧 546 §16；删除项与边界同时对齐 division-plan.md“面 3”，且不再保留特殊 forward decision。【旧 RFC 候选 | A.md 旧 546 §16 | 已纳入】
...
来源：沿用旧 546 §17，并加入 record-2 3:06—3:10 的 waiting policy 与重新声称路径。【旧 RFC 候选 | A.md 旧 546 §17 | 已纳入】【操作员原话 | record-2 3:06、3:09 | 已裁决】
...
来源：压缩沿用旧 546 §18，删除所有已失效的旧文档具体行号引用；current/target 分层继续作为证据边界。【旧 RFC 候选 | A.md 旧 546 §18 | 已纳入】
...
增长验收覆盖两种声明模式：`fixed-deadline` 在首次全体落定时固定截止点，`sliding-deadline` 在成员增长并再次全体落定后重建完整窗口；两者的期满事件都只提交一次，重放不再次等待。原位置结束后，A2-at-A typed 拒绝且不消耗事实幂等键，A2-at-B 重新声称可独立判定；开放前沿只能来自拒绝结果副产品。

交接验收覆盖 continuation 存续时的 await crash 恢复与结果恰好注入一次、continuation 丢失时的 `active loss → exception`、dependsOn 环和 value-blind 门、业务负面返回与 exception 分栏。逐级触发 retry policy、`skip-step | stop-task`、`skip-task | stop-group`、`advance-next-item | stay-on-current-item`，证明 evaluator 穷尽、跨层动作由对象域 committed transition 执行，配置穷尽时全局停机而非被 par 吸收。
...
### 两种注入模式汇入同一 typed admit

```mermaid
flowchart TB
    I[into-chain] --> E[选择既有chain]
    N[new-workspace] --> C[chain create]
    C --> F[形成允许长期存在的空active chain]
    E --> A[提交work candidate]
    F --> A
    A --> P[pinned parser]
    P -->|typed candidate| AD[面3 admit]
    P -->|rejected| R[零业务写入的typed结果]
```
...
```mermaid
flowchart TB
    DI[delivery identity 外部投递] --> MAP[消费daemon关联映射]
    RI[request identity engine请求] --> MAP
    RI --> RR[durable request record]
    WI[work identity chain加itemId] --> RR
    WI -->|成功admission后收敛重复工作| T[task identity]
```

### durable request record 的线性化

```mermaid
sequenceDiagram
    participant C as Caller
    participant E as Engine
    participant D as Durable request record
    participant O as Object domain
    C->>E: typed request加request identity
    E->>D: 查找或建立请求记录
    alt 已有确定结果
      D-->>C: 返回同一typed结果
    else 新请求
      E->>O: parser加admit事务
      O-->>D: accepted或rejected typed结果
      D-->>C: 返回durable结果
    end
```
...
```mermaid
flowchart TB
    P[固化prompt投影加typed配置] --> A[argv builder]
    P --> M[model]
    P --> E[spawn environment与sandbox]
    P --> S[session identity与resume]
    A --> X[真实endpoint effect]
    M --> X
    E --> X
    S --> X
    X --> R[result reader]
...
stateDiagram-v2
    [*] --> Probing
    Probing --> PreSpawnAbsence: spawn前确认缺席
    Probing --> ActiveRun: endpoint可用并建立run
    ActiveRun --> TerminalWinner: terminal admission先提交
    ActiveRun --> ActiveLoss: loss decision先提交
    ActiveRun --> UnknownEffect: effect无法证明且无winner
    PreSpawnAbsence --> [*]: 面3 held处置
    TerminalWinner --> [*]: 正常针眼提交
    ActiveLoss --> [*]: 面3 exception落定
    UnknownEffect --> [*]: unknown hold且不重复推进
```

### terminal 与 loss 的 durable winner

```mermaid
sequenceDiagram
    participant P as Provider boundary
    participant T as Terminal admission
    participant L as Loss decision
    participant D as Durable ordering
    par 读取terminal
      P->>T: terminal evidence
    and 检测loss
      P->>L: loss evidence
    end
    T->>D: 尝试提交terminal winner
    L->>D: 尝试提交active loss
    D-->>P: 只接受线性顺序中的唯一winner
    P-->>P: restart后重放同一winner
```
...
mindmap
  root((hook anchors))
    ...
    五时态转换点
      ...
      流转判定
    函数域针眼出口
    对象域committed transition
      稳定对象域事实
```

### anchor、delivery、execution 与 audit

```mermaid
sequenceDiagram
    participant A as Existing anchor
    participant H as Hook adapter
    participant S as Subprocess primitive
    participant D as Audit store
    A->>H: 锚点触发加只读projection
    H->>D: 建立logical delivery
    H->>S: 启动execution
    S-->>H: success nonzero timeout或signal
    H->>D: 闭合execution并关联delivery
    alt crash留下未决delivery
      H->>S: 恢复后at least once再执行
      H->>D: 记录新的execution
    end
```

### map 与 hook 共享机制但不共享权威

```mermaid
flowchart TB
    SP[共同 subprocess primitive] --> M[map]
    SP --> H[hook]
    M -->|preset声明| C[产值进入context]
    C --> R[影响检查与路由]
    H -->|operator声明| A[只写旁路audit]
    A -. 不进入context不裁决流转 .-> R
```
...
来源：A.md 第 0 篇 0.3、0.4、0.7、0.8；五时态的转换点与闭包经针眼进入、交付两侧的锚点来自 record-3 第 18 轮操作员原话；对象域 committed transition 已是公共模型中的稳定持久事实，作为只读 hook anchor 不创造新时态或 mutation 权威，因此直接纳入闭集；A.md 旧 543 第十节的零自反纪律。
...
```mermaid
flowchart LR
    G[Independent Gateway] --> S[SQLite strict status reader]
    G --> E[events history与active offset]
    G --> A[attempt artifacts]
    G --> C[typed context projection]
    G --> D[daemon typed socket]
    S --> U[Browser与PWA]
    E --> U
    A --> U
    C --> U
...
```mermaid
flowchart TB
    P[pidfile probe] --> R[原样结果加采样时间]
    S[socket connect probe] --> R
    D[daemon status RPC] --> R
    R --> U[UI并列展示]
    U -. 不合成为单一红绿灯 .-> X[unknown或分裂状态]
```
...
flowchart LR
    ...
    R --> V[context与副作用两面]
    V --> A[有限动作]
    A --> Q[重读status events audit]
    Q -->|accepted rejected failed unknown| H
```
