...
- **Recycle zone**（#452）：agent 通过 `coder-loop item update --status` 写入 admissible status 后，daemon 给它 500 秒自然退出。事件流写 `recycle.pending_entered` 起手；自然退出写 `recycle.natural_exit`；到期未退出直接 SIGKILL 进程组，事件流写 `recycle.timeout_kill`（因为 agent 已经宣告完成，SIGTERM 不再需要）。
