...
- **Row completeness** — every row of both tables appears in the results with an actual, a `deferred: e2e-replay` verdict (browser rows only — anything else deferred is a defect), or an exact could-not-execute error. Any silently absent row invalidates the replay: send back for the missing rows. Deferred browser rows are not closed here: carry them into the e2e-replay dispatch's `Step focus` and close them from that report.
...
Then form your own verdict from the accepted report: all non-deferred rows matched → those contract rows hold (browser rows close from the e2e-replay report; e2e claims and live checks also come from their own steps); any mismatch/missing artifact/broken Command → retry (cite every failing row); environment rows with no artifact and no feasible re-execution → retry or blocked per whether iteration can fix it.
