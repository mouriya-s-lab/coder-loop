...
Issue 的 deliverable 形态由 issue body 自身的内容决定——本 preset 不再用 label 编码它。planning agent 在写 issue 时按 §1.6 的标题/必备段对应关系写出形态，review/iter agent 在读 issue 时按 body 内容判断走哪条工作流。
...
iter 与 review agent 通过这套 title/body 对应关系判断 issue 的 deliverable 形态——既是 planning 端写时的 self-check，也是消费端读时的对照锚。
...
Review 是调度者（orchestrator）：PR-backed 路由必须先派 diff-audit / test-integrity / replay / e2e-replay 四个 subagent 真跑、拿齐四份已验收报告后才允许做 body 判断与 verdict；诚实性/协议判断由调度者亲自做。每次 PR 回复（retry 反馈与 accept 总结）都是完整 review 报告：全部检查的逐项摘要 + 独立 `## 缺失汇总` 区块 + `## Skipped checks` 区块（每个未跑检查写明理由）。按 `review-entry.md` 的 phase 顺序列出每个验收点（entry 与 quality/ 文件做 ground truth）：

| 验收点 | 执行方 | 输入 | 规则 | 失败处置 |
|---|---|---|---|---|
| Diff audit | diff-audit subagent 真跑（纯读） | PR diff vs base + changed code 本体 + issue body 点名的全仓收敛模式 | 每个 changed file 映射到 issue scope；runtime artifacts / scheduling state 不入仓；代码审查锚定 issue 设计：逻辑错误（须可追溯失败路径）/ 偏离 issue 声明的设计（须引原句）/ 违反项目 conventions（须引来源）/ diff 内结构缺陷；并对 issue body 自己点名的全仓收敛模式（`## 不应残留` / `## 预期结果` "升一等类型 / 不再 …" / `## 验收标准` 数值红线）做一次性全仓 site 枚举（issue 自声明 = 不算发散，每个 pattern 一行 quote + 命令 + 完整 site 清单）——其余发散性发现（替代设计、issue 未点名的 diff 外代码）不进 verdict | retry action（引用全部失败行 + 每个 pattern 的全部剩余 site） |
...
| Contract replay | replay subagent 真跑 | issue `## 验收标准` + `## 继承验证义务` 全部行 | 逐行执行/复验（列 `# / Dimension / Check / Command / Env / Expect`，actual vs Expect）；`Env=browser` 行属 e2e 域，记 `deferred: e2e-replay` 由 E2E replay 在真 UI walk 内关闭；unblock-deliverable 路由必含 blocked-path e2e；其余未跑行仅两种合法形态：setup 未完成（附尝试记录）/ manifest 缺项（计为 packet 失败） | retry action（引用全部失败行） |
...
- 实现-PR-deliverable / unblock-deliverable issue 都走 PR-backed 路由（四份 dispatch 报告 + 全套 self-judgment）；
...
- 写 queue 进 `central state DB` 后必须跑 `coder-loop status <target> --json --chain <chain-name>` exit 0，并确认 `.state.ok == true` 与 `.queue.selected.id` 指向预期 next item，否则 plan/init-queue 不能 verdict pass。
...
少数实现-PR-deliverable issue 太小（trivial 重命名 / 格式化），写不出有意义的 5 行表。允许省略 `## 验收标准`，contract replay 对表自跳过。代价：reviewer 完全靠四层证据 packet 判断，不享受 row-by-row 强制。planning agent 决定省略时必须在 issue body 写一句 "本 issue trivial，无 `## 验收标准` 表；依赖 PR 四层证据"——给 review agent 看见 explicit 决定，不是漏写。
