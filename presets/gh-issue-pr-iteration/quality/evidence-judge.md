...
- **Environment blockers must show the attempted setup.** "Could not run / environment issue" is acceptable only with the setup commands actually attempted (dependency install, build step) and their output attached; a bare skip over missing dependencies or an unbuilt tree is setup the executor skipped — send it back to do the setup and run the thing.
