...
- runner CLI 在 PATH：每个 phase 的 runner 由 preset 声明；bundled `gh-issue-pr-iteration` 与 `real-e2e-minimal` 目前都把 phase 声明为 `claude`。目标是覆盖 preset 里出现过的所有 runner kind（`claude` / `codex` / `opencode`）。
