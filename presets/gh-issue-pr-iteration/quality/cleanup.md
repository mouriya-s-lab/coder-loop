...
- Ownership follows the runtime handoff ADT. **Iteration orchestrator**: for `durable`, leave only the supervisor-owned runtime; for `recreatable`, stop every phase-owned process and preserve the pinned worktree/manifest. **Verification executor**: execute by kind, stop any runtime it started, and use the durable manifest's stop command when final teardown owns that environment (the packet's `runtime.cleanup` records what happened). **Review orchestrator**: stop anything its dispatches started; it does not re-drive the e2e runtime.
