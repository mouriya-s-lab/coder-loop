# #595 feat(cli): context 共享读取命令面——scope 过滤查询与 GUI 消费 boundary

- state: **closed**  | author: `RiriAgent`  | created: 2026-07-02T14:04:07Z  | updated: 2026-07-17T20:41:43Z
- closed: 2026-07-17T20:41:43Z  | state_reason: `not_planned`
- labels: (none)
- url: https://github.com/mouriya-s-lab/coder-loop/issues/595
...
> "agent 用 CLI 按 scope / item / since 等条件过滤查询；引擎不把 entry 内容渲染进 prompt（否则作用域过滤白设计）。经现有 doc-binding 机制注入一段「CLI 用法 + 本 run 的 scope 标识」说明（`phaseExitsEpilogue` 同款先例），注入的是用法文档，不是内容。" — #545「读取形态：拉取制」

> "读取是 agent 可用的命令面（区别于 `logs.query` 的 agent 硬拒绝——context 读是 A 域内容通道，本来就是给 agent 的），agent 凭证读自己 chain，operator 读任意 chain；新命令进 #409 的编译期穷尽分类。" — #545「entry 模型」

> "**授权无粒度，chain 内随意读写**……可见范围天然限定在凭证所属 chain。operator 无凭证路径全量读写任意 chain。" — #545 设计裁决 3

> "**RFC-5（#544，已裁）**：entries 的 read API 及其 JSON shape 归**本 RFC**——daemon socket 读命令的返回 boundary（arktype）即 GUI 消费契约，#544 纯消费不定义（其 body 同步更新）；分页/过滤维度由本 child固定为类型化过滤闭集与稳定 keyset cursor。" — #545「跨 RFC 接口假设」

#544 侧对应承诺：

> "**RFC-3（#545，已裁）**：context entries 展示面消费 #545 read 命令的 arktype boundary（shape 归 #545 拥有），本 RFC 纯消费；分页/过滤形态随其实现 child。" — #544「与其他 RFC 的接口假设」
...
- 同一 item 下一轮 run 的 agent 拉取上一轮留下的 item-scope entries（retry 轮次间传递）；后续 phase 拉取前序 phase 的 entries（phase 间传递）。
- 链内任一 agent 拉取 chain-scope 公告。
- operator 无凭证查询任意 chain 的 entries（诊断）。
- #544 GUI 的 entries 展示面以本 child 的返回 boundary 为消费契约直接渲染。

## 上下文

Repo `mouriya-s-lab/coder-loop`，基线 main@a007fa4（行号实施前自行 grep 核对）。

- 存储与 envelope shape 由 #594 钉定：v3 首版无 topic/tag；本 child 是其查询消费端，不改 envelope。
- 凭证解析：`resolveItemMutationCaller`（`src/daemon.ts:3182-3223`）产出 `operator | agent(runId, phase)` 主体——agent 读的 chain 限定从这里推导。
- #409 分类先例：`logs.query` 是 `hard-deny-for-agent`（spec `src/daemon.ts:1304`，拒绝在 `runAuthorizationGate` `1337-1391`）——本命令与之相反，须选 agent 可用的分类并进 `Record<DaemonCommandName, DaemonCommandSpec>` 穷尽映射（`1275-1307`）。
- boundary 先例：`status --json` 经 `StatusSnapshotBoundary`（arktype）输出稳定快照——read 命令返回 boundary 照此做 GUI 契约。
- append-only 存储（地基 child 性质 3）：entries 只增不改不删，游标分页无删改漂移问题。

## 问题

地基 child 落地后 entries 只写不可读：agent 侧无任何拉取命令，#545「拉取制」读取形态没有载体；#544 的 entries 展示面没有可消费的 read boundary（其接口假设明文等待本 RFC 的实现 child）。
...
- 本 child 范围内：boundary 外的过滤参数；agent 路径上任何绕过凭证 chain 限定的查询入口；返回 shape 以匿名 `"object"` 槽或裸 JSON 透传。
- 本 issue 范围之外不应改动：envelope shape 与写入面（归地基 child）；group 键推导（归 group 真实化 child）；prompt 注入的用法文档内容（归执法 child，经 #553 `toolRequirementsDoc` 承载）；GUI 展示面本身（归 #544）。
...
- 授权无粒度（#545 裁决 3）：不引入 per-scope / per-phase 读授权——scope 是寻址/过滤维度，不是可见性边界；唯一可见性边界是 chain。
- 与 #534 audit 树排序默认（v3 总控整合裁定，2026-07-02）：触 `src/daemon.ts` 命令面，默认 #535/#536/#538 先合、本 child 其后 rebase；偏离需在本 issue 说明理由。
...
- Depends on: #594（存储与写入面——envelope shape 与 entries 的事实源）。
- Blocks: #597（「必须调用」执法——注入用法文档须覆盖读写两面）、#598（收尾对齐）、#583（context entries 展示面——纯消费本 child 的 read boundary）。
...
### comment #5007302853 by `RiriAgent` — 2026-07-17T20:41:43Z

重新拆分后由 #730 承接。旧 issue 无关联 PR，关闭。
...
- 2026-07-02T14:04:08Z `assigned` @RiriAgent
...
- 2026-07-02T14:05:20Z `parent_issue_added` @RiriAgent
- 2026-07-02T14:05:39Z `cross-referenced` @RiriAgentsrc=545
- 2026-07-05T07:48:36Z `cross-referenced` @RiriAgentsrc=583
...
- 2026-07-17T20:41:43Z `commented` @RiriAgent
- 2026-07-17T20:41:44Z `closed` @RiriAgentcommit=None