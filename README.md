...
Runner 默认继承启动宿主：Codex 里启动 loop 就默认用 `codex`，Claude Code 里启动就默认用 `claude`。target config 可用 `"runner": "claude" | "codex"` 覆盖默认值，单个 queue item 也可用同名 `runner` 字段覆盖；`doctor` / `status --json` 会显示实际选择。
